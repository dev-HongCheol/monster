...
- [ ] **다중 슬롯(주 검증):** `SpellCaster.startingSpellIds`를 3색 로드아웃(`["fireball","ice_missile","lightning_bolt"]`)으로 두면 발사체가 슬롯별 올바른 **분류색**(fire=주황빨강/ice=하늘/lightning=노랑)·방향으로 나간다. 풀이 3색 노드를 섞어 재사용하므로 색/방향 잔류가 있으면 즉시 드러난다.
- [ ] ~~부채꼴(count≥2) 발사 시 발사체 수·방향·분류색 정확~~ → **현재 인게임 트리거 없음**: 전 마법 `projectileCount=1`이고 "발사체 수 +1" 강화 미배선(엔진 `buildFirePlan`은 count≥2 부채꼴 지원, #19). 부채꼴 회귀를 보려면 `spells.json`의 `projectileCount`를 임시 `≥2`로 → 좌우 대칭 N발·동일 분류색 확인 후 되돌린다. (발사체 수 강화 슬라이스 들어오면 정식 항목화.)
