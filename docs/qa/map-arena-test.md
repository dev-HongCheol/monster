...
- [ ] **벽 근처 발사체 발사 (회귀 방지)** — 네 벽·모서리 근처로 이동해 마법을 발사했을 때 발사체가 즉시 사라지지 않고 정상적으로 날아가 적에 명중한다. 적 발사체도 그 영역에서 정상 동작한다. (발사체가 아레나 절대좌표에 스폰되는데 원점 기준 화면 컬링에 잘려 외곽 링에서 전투가 죽던 회귀 — 리뷰 #1, `isOutsideArena`로 수정.)
...
- [ ] **크로스머신 diff(F9)** — Camera의 `orthoHeight` **에디터 값**이 360으로 고정돼 있고(코드 할당은 런타임 변동만 막지, 직렬화 값은 에디터 값으로 박힌다), 저장 후 `git diff main.scene`에 `_orthoHeight` churn이 재발하지 않는다.
