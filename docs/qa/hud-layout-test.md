...
| **main.scene (UI Canvas)** | HP/XP ProgressBar 노드·레이아웃 앵커(Widget) 추가 + 기존 HpLabel을 **HP 바 위 중앙 오버레이**로 재배치. placeholder 4종은 **이번 슬라이스 제외**(→ backlog J4). 7단계 사용자 작업. | 아래 §3~§5. |
...
> **재검증 근거(2026-07-03, HP 숫자만 포맷 + placeholder 이월 리워크):** `hud.hp`를 `{cur}`(숫자만)로 축소 + HudController `max` 인자 제거(코드/데이터 변경만, 테스트 무추가) — 피처 테스트 22/22 + 전체 스위트 386/386 통과(`start-verification` GREEN). 통과 커밋 SHA는 아래 리워크 커밋.
...
> **부모 계층 (중요):** 새 HUD 노드는 UICanvas 바로 아래가 아니라 **`HUD` 노드 아래**에 둔다 — 실제 씬 계층은 `UICanvas > HUD > {HpLabel·WaveLabel·TimerLabel·LevelLabel·XpLabel}`이고 `HudController`는 **HUD 노드**에 붙어 있다. 새 `HpBar`·`XpBar`는 기존 라벨과 **형제**(HUD의 자식)로 만들고, 기존 `HpLabel`은 `HpBar`의 **자식**으로 옮겨 바 위 중앙에 겹친다(§4.1). placeholder는 이번 슬라이스에서 제외(§4.2). (`main.scene`은 게임/UI 두 Canvas로 분리 — card-layer-fix.)
>
...
| 영역 | 요소 | 앵커 | 세로 구간(대략) |
|---|---|---|---|
| 좌상 | WaveLabel | L24 · **T24** | y 24 ~ 44 |
| 좌상 | TimerLabel | L24 · **T56** | y 56 ~ 76 |
| 좌하 | HpBar | L24 · **B44**, 200×18 | 아래서 44 ~ 62 |
| 좌하 | HpLabel (HpBar 위 중앙 오버레이) | HpBar 중앙 | HpBar와 동일 밴드 |
| 좌하 | LevelLabel | L24 · **B16** | 아래서 16 ~ 36 |
| 하단풀폭 | XpBar | L0·R0·B0, 1280×12 | 아래서 0 ~ 12 |

> **좌상 세로 스택:** Wave(Top 24) → Timer(Top 56). placeholder 미니맵을 이번 슬라이스에서 제외(→ backlog J4)해 좌상단이 비므로, Wave/Timer는 **상단 모서리(24/56)** 에 그대로 둔다(실제 씬 값과 일치). 세 라벨(Wave·Timer·Level)은 **앵커 X=0(왼쪽)** 이어야 문자열 폭이 바뀌어도 좌측 정렬이 유지된다(§5 주의).
> **좌하 세로 스택:** XP바(0–12) → Level(16–36) → HpBar(44–62). 겹침 없음. HpLabel은 HpBar 위에 **겹쳐(중앙 오버레이)** 별도 세로 구간을 차지하지 않는다. (HpBar를 예전 Bottom=24로 두면 Level(16–36)과 겹치므로 **44로 올림**.)
...
|---|---|---|---|---|
| `HpBar` (HUD) | `cc.ProgressBar` (Mode=HORIZONTAL, Bar Sprite=흰 텍스처, 배경 노드에 `COLORS.BAR_BG` 톤) | ☑Left=24 · ☑Bottom=44 (레벨 라벨·XP 바 위) | 폭 ≈ 200 · 높이 ≈ 18 | `@property hpBar`에 연결. `barSprite`(Fill) 지정 필수 — 코드가 이 스프라이트를 틴트. 바 위 중앙에 `HpLabel` 오버레이(아래 행). |
| `HpLabel` (**HpBar 자식**) | `cc.Label` (기존, 재배치) | Widget 없음 — Position (0, 0) | 앵커 **(0.5, 0.5)** · H/V Align=**CENTER** | 기존 `hpLabel`. HpBar 중앙에 겹쳐 **현재 HP 숫자만** 표시(`hud.hp`=`{cur}` → `formatNumber(ceil(playerHp))`, 예 `205`/`1,205`). 빨간 바 위 가독 위해 흰색+아웃라인 권장. 중앙정렬이라 폭 변화에도 안 흔들림. |
...
| `WaveLabel` (HUD) | `cc.Label` (기존, 좌상단) | ☑Left=24 · ☑Top=24 | 라벨 자동 | 기존 `waveLabel`. `hud.wave`. 상단 모서리(placeholder 미니맵 이월로 위가 빔). **앵커 X=0**(§5 주의). |
| `TimerLabel` (HUD) | `cc.Label` (기존, 좌상단) | ☑Left=24 · ☑Top≈56 (웨이브 아래) | 라벨 자동 | 기존 `timerLabel`. `formatTimer`로 `mm:ss` 카운트다운. **앵커 X=0**(§5 주의). |
...
- [ ] HP가 닳으면 **HP 바(좌하단)가 값에 비례해 줄어든다**. **바 위 중앙에 현재 HP 숫자만** 겹쳐 표시된다(접두사 `HP:`·최대치 없음 — 예: `205`). 값이 1000 이상이면 **천단위 콤마**(`formatNumber`; 예: `1,205`. 1000 미만이면 콤마 없음). 라벨은 흰색+아웃라인 권장(빨간 바 위 가독).
...
- [ ] HP가 0이 되면 **result(결과) 씬으로 전환**되고 RETRY/MENU 버튼이 뜬다(기존 동작 회귀 없음 — main의 gameOverPanel이 아니라 별도 result 씬). ⚠️ 사망이 즉시 씬 전환이라 HP 바가 0으로 비는 장면은 거의 안 보인다(0.5초 틱 데미지 + 즉시 result 로드) — **의도된 현 동작이며 연출 개선은 backlog로 이월**(F26 사망/게임오버 연출).
...
- ~~placeholder가 자리에 보인다 / 클릭 반응 없음~~ → **이번 슬라이스 제외**: placeholder UI(미니맵·보스HP바·메뉴·스킬그리드)는 다음 UI 슬라이스로 이월(§4.2, backlog J4).
