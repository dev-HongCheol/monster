...
- **리뷰 커밋:** 1차 `8ee72f2`(base) → `2551e3c` / 2차(재리뷰) → `4af4896`
...
- **범위 주의:** 이건 **이 슬라이스 이전부터 있던 결함**이다. 사망 연출이 끝나 `_returnToPool`이 도는 시점도 프레임 도중이라 같은 창이 열린다. 다만 재활용이 그 빈도를 크게 올린다.
...
- **조치:** 코드는 그대로 두고, 이 자기 제한을 **속성 테스트로 못박았다**(폴백이 발생하는 영역 전체를 훑으며 사후조건 ③을 단언). 나중에 누가 폴백을 건드리면 테스트가 잡는다.
