...
- [ ] **접근 → 사거리 대기 → 휘두르기:** 근접 적이 추격으로 붙다가 사거리 안에 들면 **멈춰 서서**(플레이어에 파고들지 않고 사거리에서 대기) 윈드업 → 1회 가격 → 쿨다운 뒤 플레이어가 벗어났으면 재접근하는 루프가 보인다.
- [ ] **겹침 없음:** 공격·쿨다운을 반복해도 근접 적이 **플레이어와 위치가 겹치지 않는다**(사거리 홀드 확인). 겹쳐서 회피 불가 연타가 되지 않는다.
- [ ] **부채꼴 섹터 마커:** 윈드업 동안 잠근 조준 방향으로 **파이 조각(호) 모양** 범위가 뜨고, 꼭짓점이 적 중심에 붙는다. **칠해진 빨간 영역이 실제 위험 콘**이다(여집합/안전지대가 칠해지면 fill 반전 — arc 방향 확인). 각도·사거리가 적별로 다르다(두억시니 넓고 짧게 150° / 그슨대 좁고 짧게 90°). 섹터가 실제 명중 판정 범위와 일치한다.
