...
- [ ] Cocos Creator로 프로젝트를 연다. **자산 누락·컴파일 에러 경고가 없는지** 확인한다 (`I18nKeyGuard.ts`가 `game/assets/`에서 빠졌지만 참조하는 게임 코드가 없으므로 조용해야 한다).
- [ ] 콘솔에 빨간 에러가 없다.

### 5-2. 인게임 한 바퀴

- [ ] 게임을 시작한다 → 플레이어가 움직이고 마법이 발사된다.
- [ ] 적을 처치해 XP를 먹고 **레벨업 카드가 뜬다** → 카드를 고르면 패널이 닫히고 게임이 재개된다.
- [ ] ESC로 일시정지 → 재개된다.
- [ ] 죽을 때까지 맞는다 → **HP가 0이 되면 실제로 죽고** 결과 화면으로 넘어간다.
- [ ] 결과 화면에서 **RETRY(재시작)** → 씬이 리로드되고 게임이 정상적으로 다시 시작된다. **싱글톤이 실제로 null이 되는 유일한 순간이므로 이 항목이 가장 중요하다.**
- [ ] 재시작 후에도 마법 발사·XP 획득·레벨업이 정상 동작한다.
...
- [ ] 다른 머신에서 클론한 직후(= Cocos로 아직 안 연 상태) `pnpm typecheck`를 실행한다 → **TS5083 같은 날것의 에러가 아니라 사람이 읽는 안내**가 떠야 한다. 테스트 프로젝트는 정상적으로 검사된다.
