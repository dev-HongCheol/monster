...
4. **크기(contentSize)** — `UITransform`:
   ...
   - **확정 (채택 이미지 `player_mage_bridge.png` = 788×1012):** 종횡비 788:1012(≈0.78)를 보존해 **`contentSize = 100 × 128`**로 시작한다(전신 약 128px 높이, 몸통 코어 대략 45~50px라 충돌 지름 50px과 맞음). 화면에서 커/작아 보이면 이 비율(100:128)을 유지한 채 값만 조정한다(예 88×113, 112×144). 반드시 contentSize로 조정하고 노드 스케일은 (1,1,1) 유지.
...
| `Player` > `UITransform` > `Content Size` | **100 × 128** (종횡비 788:1012 보존, 몸통코어≈50px) | ❌ → 사용자 |
| `Player` > `Scale` | (1,1,1) 유지 | ❌ → 사용자 |

## 수동 테스트 체크리스트

- [ ] 게임을 켜면 플레이어가 **파란 상자가 아니라 마법사 그림**으로 보인다.
- [ ] 그림이 **정사각으로 눌리지 않았다**(종횡비 정상).
- [ ] 그림이 논리 위치에 **정렬**돼 있다(적 접촉·피격 지점이 그림 중심과 어긋나지 않음).
- [ ] **엠버/발광 엣지 알파가 깔끔**하다(검은 테두리·잘린 반투명 없음).
- [ ] 스타일이 **F58b 애니 셀 룩**과 일치한다.
- [ ] **게임플레이 불변 확인:** 이동 속도, 적과의 충돌 판정, 경험치 픽업 반경이 교체 전과 동일하다(그림만 바뀌고 수치 무변경).
- [ ] 카메라가 플레이어를 그대로 따라간다.
