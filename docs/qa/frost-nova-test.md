...
- [x] frost_nova 보유 시 **쿨다운마다 플레이어 중심에서 노바가 발동**한다(발사체가 날아가지 않음).
- [x] 노바 반경 안 **여러 적이 한 번에** 피해를 받는다(플레이어 주변 군집).
- [x] 반경 **밖** 적은 피해를 받지 않는다.
- [x] **적이 한 마리도 없어도** 노바는 쿨다운마다 발동한다(자기중심 — 발사 보류 없음). [핵심]
- [x] 노바가 빙결·슬로우를 걸지 **않는다**(순수 피해 — 적이 멈추거나 느려지지 않음).
- [x] 발동마다 **노바 VFX가 플레이어 위치에** 표시된다.
- [x] 범위 강화 카드를 고르면 **노바 반경이 눈에 띄게 커진다**(VFX 링도 비례 확대).
- [x] 강화 카드 패널에 **프로스트 노바 범위 카드**가 등장한다(라벨 "범위", i18n `upgrade.range`).
- [x] 강화 카드 패널에 **프로스트 노바 발사체 수 카드가 등장하지 않는다**(자기중심 — 제외).
- [x] 강화 카드 패널에 **프로스트 노바 지속시간 카드가 등장하지 않는다**(순수 피해 — 제외).
- [x] 데미지·쿨다운 강화 카드는 정상 등장하고 적용된다.
- [x] 발사체 마법(파이어볼 등)과 함께 보유 시, **발사체 마법은 기존대로** 적을 조준해 발사된다(노바 분기가 발사체 경로를 깨지 않음).
