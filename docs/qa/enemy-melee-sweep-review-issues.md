...
- **처리:** 잠정 로버스트니스 항목이라 즉시 수정 대신 **백로그로 이월**(현 트리거 없음, 프로젝트 로버스트니스 정책). → `docs/development/backlog.md` **F25**. 닫을 때: `coneHitsTarget`에 "무제한 각 허용(판정이 정본, 마커는 렌더용 근사)" 주석 + 데이터 테스트에 `coneAngleDeg ≤ 178` 상한 단언으로 마커·판정을 실 데이터에서 증명적으로 동기화.
