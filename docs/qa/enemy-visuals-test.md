...
### 시각 구분 (tint / scale)
- [x] 웨이브 진행 시 **3가지 외형이 한눈에 구분**됨: 표준=기본색·중간 / 스워머=밝은 청록·작음 / 탱크=어두운 보라·큼
- [x] 같은 종류는 항상 같은 색·크기로 스폰됨 (데이터 일관성)
- [x] 시각 크기(scale)와 **충돌 판정은 별개** — 탱크가 커 보여도 접촉 반경은 데이터값(collisionRadius)대로 동작 (작은 스워머에 가까이 가야 데미지)

### 피격 플래시
- [x] 마법 투사체가 적에 명중하면 **흰색으로 순간 점멸** 후 원래색 복귀
- [x] 연속 피격 시 플래시가 매번 리셋되어 다시 번쩍임
- [x] 플래시 후 색이 **원래 tint로 정확히 복귀**(색이 영구히 밝아지거나 어두워지지 않음)

### 사망 연출 (팝 + 페이드)
- [x] 적 사망 시 즉시 사라지지 않고 **살짝 부풀었다(팝) 투명해지며(페이드) 소멸**
- [x] **(리뷰 I-1) 페이드가 실제로 보이는지 확인** — 알파 페이드는 Sprite 머티리얼이 알파 블렌드여야 보인다. 팝만 보이고 페이드 없이 갑자기 사라지면 Enemy.prefab Sprite의 머티리얼을 기본(builtin-sprite)으로 설정
- [x] 사망 연출 중인 적은 **플레이어에게 접촉 데미지를 주지 않음**(이동도 멈춤)
- [x] 사망 연출 중인 적은 **투사체에 다시 맞지 않음**(중복 처리 없음)
- [x] XP 아이템이 **정확히 1회** 드롭됨 (연출 중 중복 드롭 없음, 사망 위치에 생성)
- [x] 연출 종료 후 노드가 완전히 제거됨 (씬에 잔존 노드 없음)

### 회귀 (기존 동작 유지)
- [x] 스폰·추적·접촉 데미지·웨이브 스케일링 정상 (S1 동작 유지)
- [x] XP 획득 → 레벨업 → 카드 선택 흐름 정상
