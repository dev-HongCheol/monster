...
- [x] **이무기 부채꼴:** 전방으로 여러 발이 부챗살 모양으로 퍼져 플레이어 쪽을 향해 날아간다.
- [x] **물귀신 확산:** 한 번에 사방 360°로 탄막이 깔린다(조준 방향과 무관하게 전 방향).
- [x] **물귀신 정지형:** 거의 제자리에 머물며 쏜다(kite 데드존 정착). 제자리 탄막이 플레이어에게 이동을 강제하는지.
- [x] **텔레그래프:** 부채꼴·확산도 발사 직전 단일 윈드업 점멸이 정상으로 뜬다(타입 무관 동일 텔레그래프).
- [x] **다중 피해 묶임:** 한 볼리의 다발이 같은 순간 플레이어에 닿아도 피해는 틱당 1회(가장 센 1발)만 — player-iframe과 정합.
- [x] **풀 확장:** 물귀신 확산(예: 8발)이 동시에 나가도 발사체 풀이 정상 확장되고 프레임 드랍이 없다.
- [x] **구미호 회귀:** 단발 발사체 적(구미호)이 S2a와 동일하게 동작한다(N발 배선이 단발을 깨지 않음).
- [x] **마법 회귀:** 플레이어 마법의 부채꼴 발사(파이어볼 등)가 기존과 동일하다(`directionalPlan` 추출 영향 없음).
- [x] **스폰:** 이무기·물귀신이 실제 웨이브에서 스폰돼 등장한다.
