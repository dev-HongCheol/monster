...
| `components/XPItemController.ts` | 아이템 드롭 위치, 픽업 반경 감지, 흡수 후 소멸 |
...
| `data/GameTypes.ts` | `GameState.Victory`, `GameResult.gameVictory` 추가 |
| `components/EnemyController.ts` | 적 사망 시 XPItem 스폰, playerNode 전달 |
...
| XPItemController | xpValue: 10, pickupRadius: 50 |
...
- [ ] 적 처치 → 해당 위치에 XP 아이템 스폰
- [ ] 플레이어가 XP 아이템 반경(기본 50u) 내 진입 → 자동 흡수
- [ ] 흡수 후 XP 아이템 노드 소멸
- [ ] HUD XpLabel 수치 증가 확인 (`XP: n / 100`)

### 레벨업

- [ ] XP 100 채우면 레벨업 → 카드 선택 패널 표시
- [ ] HUD LevelLabel `Lv.1` → `Lv.2` 증가
- [ ] 레벨업 후 requiredXp 증가 확인 (`XP: 0 / 120`)
- [ ] 초과 XP 이월 확인 (110 XP 수집 시 레벨업 후 `XP: 10 / 120`)
- [ ] 레벨 3, 4에서도 동일하게 동작 (배율 120% 누적 확인)

### 카드 선택 후 게임 재개

- [ ] 카드 선택 → 패널 닫힘, 게임 재개
- [ ] 재개 후 WaveTimer 3분으로 리셋
- [ ] 재개 후 새로운 적 스폰 재개

### 웨이브 시스템 (WaveManager 변경 검증)

- [ ] 타이머 3분 카운트다운 표시 (기존 30초 → 180초)
- [ ] 타이머 0 → **카드 선택 패널이 뜨지 않음** (WaveClear 트리거 제거 확인)
- [ ] 타이머 0 → Wave 번호 1 증가 (Wave 1 → Wave 2)
- [ ] Wave 2에서 적 스폰 간격/최대 수 변화 확인

### HUD 표시

- [ ] `Lv.1` / `XP: 0 / 100` 초기값 표시
- [ ] TimerLabel이 `15:00`으로 시작 후 MM:SS 형식으로 카운트다운
- [ ] HP, Wave 기존 표시 정상 (회귀 없음)

### 승리 조건

- [ ] 전체 타이머 0 도달 → result.scene 이동
- [ ] result.scene에 `승리! N웨이브 도달` 표시
- [ ] 패배(HP 0) 시 result.scene에 `N웨이브 도달` (승리 문구 없음)

### 데이터 연동

- [ ] `experience.json` baseXp: 100 반영 (첫 레벨업 XP 100)
- [ ] `experience.json` xpMultiplier: 1.2 반영 (레벨 2 요구 XP 120)
