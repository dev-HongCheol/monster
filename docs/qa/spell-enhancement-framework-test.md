...
- **적용 경로:** `SpellCaster`가 발사 시 `DeckManager.damageFactor(spell)`/`cooldownFactor(spell)`(3-tier 포함)를 사용.
...
- [ ] 레벨업 카드 패널에 "선택 마법 강화"(보유 마법명 + 데미지/쿨다운) 카드가 등장한다
- [ ] 레벨업 카드 패널에 "분류 강화"(화염/얼음/번개 + 데미지/쿨다운) 카드가 등장한다
...
- [ ] 전역 `damage_boost`/`cooldown_reduce` 카드는 여전히 모든 마법을 강화한다(단, 효과 수치가 대폭 작아짐 — 개별/분류보다 체감 작음)
- [ ] "마법 추가" 카드는 기존대로 미보유 마법을 슬롯에 추가한다
- [ ] "생명력 강화"(hp_up) 패시브는 기존대로 최대 HP를 올린다
- [ ] 신규 마법 추가 후 해당 마법의 개별 강화 카드가 다음 레벨업부터 등장한다
