...
- [x] ~~`pnpm wf start docs-hygiene`을 다시 실행한다~~ — **이 항목이 틀렸다(2026-08-19).** `wf start`는 phase 가드가 없어 전체 상태를 `planning`으로 되돌리므로, 통과 경로를 이렇게 확인하면 진행 중인 슬라이스의 검증 넷과 정본 선언이 지워진다. 실제로 그렇게 났고 `approve-pr`이 막혀 상태를 커밋에서 복원했다. 통과 경로는 아래 E2E가 이미 덮는다(`기준점이 origin/main을 담고 있으면 통과시킨다`)
