...
| `tests/helpers/LinkCheck.ts` | 뒤쪽 212줄(`CANON_ALIASES`·`ATTRIBUTION_VERBS`·`INLINE_QUOTE`·`findInlineCanonQuotes`와 그 보조 넷)을 걷어낸다 | **링크 검사 39건이 한 건도 안 줄고 그대로 통과하는가** — 앞쪽 326줄은 한 글자도 바뀌지 않아야 한다 |
...
**통과 근거(2026-08-19):** 전체 스위트 46파일 816/816(+스킵 1), 타입체크 범위 `full`, biome 종료코드 0, `check-links`·`check-docs`·`check-qa` 전부 통과. 이 슬라이스는 피처 테스트가 없다(`wf skip-test` 사유는 상태 파일에 있다) — 회귀는 `DocLinks` 38건과 `ClaudeMdSplit` 56건이 받는다.
...
**(확정)** 상한은 38,000자다. 현재 37,845자라 여유가 155자뿐인데, 이것은 의도한 것이다 — 다음에 무엇을 「항상 읽는다」로 올리려면 같은 분량을 덜어내야 한다.
