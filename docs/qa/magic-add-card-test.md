...
- [x] 레벨업(적 처치로 XP 채움) 시 카드 패널에 **"마법 추가" 카드(미보유 마법 이름)** 가 강화/패시브 카드와 섞여 등장할 수 있다
- [x] "마법 추가" 카드를 고르면 **해당 마법이 즉시 자동 발사 대열에 합류**한다(새 분류 색 발사체가 추가로 날아감)
- [x] 이미 보유한 마법은 "마법 추가" 카드로 다시 등장하지 않는다
- [x] 강화/패시브 카드를 고르면 기존과 동일하게 적용된다(회귀 — damageMult/cooldownMult/maxHpBonus)
- [x] 카드 선택 후 게임이 정상 재개되고 웨이브/HP/이동이 기존과 동일하게 동작한다(레벨업 일시정지→재개 회귀)
- [x] (참고) 마법 3종을 모두 보유하면 더 이상 "마법 추가" 카드가 나오지 않고 강화/패시브만 나온다
