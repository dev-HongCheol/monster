...
| B2 | ⚖️ | 전역 강화 트랙 cap 검토 | 개별·분류 곡선은 cap 4인데 전역(`addGlobal`)은 무한 누적 → "개별>분류>전역" 위계가 1픽 기준은 보장돼도 다수 누적 시 점근적으로 깨질 수 있음. **→ result-stats 슬라이스가 전역에 옵션별 레벨 상한을 도입해 처리(레벨 브레이크다운 표시의 전제 — 계획 `sessions/2026-07-06-result-stats-plan.md` §2 IN·§3).** | `../qa/spell-enhancement-framework-review-issues.md` #4 | 처리 중(result-stats) |
...
| F25 | 🐛 | `HudFormatLogic` 비유한 입력 가드 — `formatTimer(NaN)→"NaN:NaN"`·`formatTimer(Infinity)→"Infinity:NaN"`·`barRatio(NaN,·)→NaN`(→`ProgressBar.progress`로 전파)·`formatNumber(Infinity)→"Infinity"`·`formatNumber(1e21)→"1e+21"`(String()가 지수표기로 전환돼 그룹핑 깨짐) | 현재 호출자(`gameTimer`·`playerHp/maxPlayerHp`·`currentXp/requiredXp`)는 전부 유한·1e21 미만이라 **미발현**. `max=Infinity`는 이미 0 반환으로 처리됨. 세 함수에 `Number.isFinite` 가드(→`"00:00"`/`0`/`"0"`) + 순수 테스트 추가. FireGeometry R1 가드와 같은 결. | `../qa/hud-layout-review-issues.md` M2 (코드 리뷰 2026-07-02) + R3 (rework 재리뷰 2026-07-03) | 낮음(미발현) |
| F26 | 🐛🎨 | **사망→게임오버 연출 — 바가 0으로 비는 비트 없이 result 씬 직행 + `gameOverPanel` 죽은 코드 + `HudController` executionOrder 부재** | HP 0 도달 시 `GameManager._applyDamage`가 곧바로 `goToResult()`(`director.loadScene('result')`)로 별도 result 씬(RETRY/MENU)을 로드한다. 피해는 0.5초 i-frame 틱 단위(뭉텅이)라 마지막 틱이 양수 HP를 한 방에 0으로 만들고 즉시 씬이 바뀌어 **HP 바가 0으로 비는 장면을 볼 수 없다**(사용자 인지: "바가 안 빈 채 RETRY가 뜸"). 곁가지 둘: (a) main의 `gameOverPanel`(+`restartButton`/`menuButton`)은 result 씬이 즉시 로드돼 뜰 틈이 없는 **사실상 죽은 코드**(QA 문서가 과거 "게임오버 패널이 뜬다"고 한 것과 불일치 — hud-layout에서 실동작으로 정정). (b) `GameManager`·`HudController` 모두 `@executionOrder` 미지정이라 HUD가 게임로직보다 먼저 도는 프레임이면 바가 1프레임 낡아 사망 프레임에 직전 HP를 그린다. **처리 방향(택):** ① 사망 시 짧은 비트(바 0으로 비운 뒤/`gameOverPanel`을 잠깐 보인 뒤) result로, ② `HudController`에 executionOrder로 사망 프레임 0 반영, ③ gameOverPanel 죽은 코드 제거 또는 result 대체 UI로 승격. **J2(게임 플로우: 게임오버→결과)와 함께 닫는 게 합리적.** | 이 대화(2026-07-03), feat/hud-layout 7단계 인게임 테스트. `GameManager.ts:210`(`_applyDamage`)·`ResultController.ts:22`·`HudController.update` | ✅완료(feat/death-flow, 2026-07-05) |
| F27 | 🔧 | **IDE(VS Code) TS ↔ Cocos 번들 TS 불일치 — 매니저 싱글톤 `static instance!:`가 VS Code TS 5.x에서 TS1255** | VS Code TS 서버(및 standalone TS 5.5)는 static 필드의 definite assignment `!`를 `TS1255: A definite assignment assertion '!' is not permitted in this context`로 막지만, Cocos Creator 3.8.8 번들 TS는 허용해 실빌드·게임 실행이 정상이다. 6개 매니저(`SpellCaster`·`DataManager`·`DeckManager`·`ExperienceManager`·`GameManager`·`WaveManager`)가 공유하는 기존 패턴이며, `mcp__ide__getDiagnostics` 기반 `pass ts` 게이트가 **매니저 싱글톤을 편집하는 슬라이스마다** 이 오탐을 표면화한다(IDE는 편집한 파일만 활성 분석 — 그래서 편집 안 한 매니저는 `[]`로 보임). 해소책(택): (a) VS Code workspace TS 버전을 Cocos 번들 TS에 정렬, (b) 싱글톤 선언을 TS1255 비유발 형태(`static instance: T = null!` 등)로 6곳 통일, (c) 게이트/린트에서 이 특정 진단 화이트리스트. | death-flow `pass ts` (2026-07-04) | 중 |
| F28 | 🎨🔧 | **죽음/승리 비트 연출 강화 — `DeathSequence` seam 위 layering (`GameManager` 불변)** | death-flow가 연출 seam(`DeathSequence.play(onComplete)`)을 깔고 가장 간단한 오버레이 페이드 하나만 넣었다(렉 느낌 해소가 목표 — 완전 정지 프레임이 행/렉처럼 보이던 것을 지속 모션으로 대체). 후속 필(feel) 강화는 전부 이 컴포넌트 안에서 얹으며 `GameManager`는 안 건드린다. (a) 순간 붉은 플래시(t=0 펀치), (b) 카메라/캔버스 셰이크, (c) 슬로모션 — 전역 타임스케일이라 시스템별 dt 스케일 배관 필요(현 freeze가 `director.pause`가 아니라 시스템별 Playing 가드라 별도 설계), (d) 채도 감소(그레이스케일), (e) 플레이어 사망 애니메이션(아트 선행), (f) **씬 로드 플래시 가리기** — `DeathOverlay`는 `director.loadScene` 시 현재 씬과 함께 파괴돼 완전 암전 뒤에도 로드 순간의 한 프레임 플래시는 못 덮음 → 오버레이 persist(씬 전환 생존) 또는 result 씬을 검은 화면에서 페이드인, (g) **승리 비트 대칭** — 승리 경로(`gameTimer` 0)는 "바가 비는" 문제가 없어 현재 즉시 `goToResult()`이나, 대칭 연출이 필요하면 같은 seam(`DeathSequence`/신규 `VictorySequence`)을 재사용. 하나가 슬라이스 분량이 되면 plan으로 승격. | death-flow 설계 개정(2026-07-05), `ui/DeathSequence.ts`·`systems/GameManager.ts` | 낮음 |
| F29 | ✨🔧 | **에디터 배선 컴포넌트의 필수 `@property` 누락 loud-fail 정합화** — `PauseController.onLoad`는 `pausePanel` 미연결 시 `console.error`/disable 없이 조용히 no-op이라, 7단계 배선 실수 시 "ESC로 얼어붙되 메뉴 안 뜸" 증상이 조용히 난다. `HudController.onLoad`(필수 prop 누락 시 `console.error` + `this.enabled=false`) house 패턴으로 맞추면 배선 누락이 크게 드러난다. 올바로 배선된 사용엔 무영향이라 pause-menu 슬라이스에선 미적용. 향후 에디터 배선 컴포넌트에 공통 적용할 규칙으로 승격 검토. | `../qa/pause-menu-review-issues.md` O1 (코드 리뷰 2026-07-06) | 낮음 |
| F30 | 🔧 | **`tsconfig` `lib`가 `target: ES2015`에 묶여 ES2017 API가 TS2550 — `Object.entries` 사용처가 전부 타입 에러 + implicit-any 파생** | Cocos가 생성하는 `game/temp/tsconfig.cocos.json`이 `target: "ES2015"`이고 `lib` 오버라이드가 없어, ES2017에 추가된 `Object.entries`가 타입에 없다(`TS2550: Property 'entries' does not exist on type 'ObjectConstructor'`). 더 나쁜 건 파생 효과다 — `Object.entries`가 `any`를 반환하는 것처럼 취급돼 `SpawnDirectorLogic.ts:46,71`에서 `reduce` 콜백 파라미터가 `TS7006`/`TS7031` implicit-any로 줄줄이 터진다(즉 **가중 스폰 계산 구간의 타입 안전성이 사실상 꺼져 있다**). 런타임 Chromium엔 `Object.entries`가 있어 실행은 정상이라 여태 미발현. 현 사용처: `SpawnDirectorLogic.ts`(2), `DebugEnhancementSeed.ts`(3), `GameManager.ts:310`. **해소책:** `game/tsconfig.json`의 "custom configuration" 절에 `"lib": ["ES2017", "DOM", "DOM.Iterable", "ScriptHost"]` 추가(`lib`은 타입 체크만 바꾸고 emit엔 무영향, `temp/`는 에디터가 재생성하므로 건드리지 않는다). 적용 후 `SpawnDirectorLogic`의 implicit-any 4건이 실제 타입으로 좁혀지는지 확인할 것. F27과 함께 처리하면 `pass ts` 게이트가 비로소 신호가 된다. | `../qa/result-stats-review-issues.md` 재검증 (코드 리뷰 재검증 2026-07-10) | 중 |
| F31 | ♻️🐛 | **결과 스냅샷의 정합 가드 2개가 cc 계층에 묶여 무테스트 + 두 가드의 실패 모드가 엇갈림** | result-stats의 C1 수정이 정합 가드를 순수 함수에서 cc 계층으로 옮기면서 순수 테스트 2건이 사라졌다. 현재 `GameManager._snapshotResult`(getEnemy=null 킬 스킵)와 `DeckManager.resultSpellSnapshots`(getSpell=null 마법 스킵)는 `DataManager.instance` static에 묶여 vitest로 못 태운다. 두 분기 모두 런타임 id가 가드가 대조하는 JSON에서 나오므로 사실상 도달 불가한 방어 코드라 위험은 낮다. **해소책:** `buildSpellSnapshots(ownedIds, getSpell, getLevel…)` 같은 순수 헬퍼로 추출하면 `getSpell=null` 회귀 테스트가 거의 공짜로 돌아온다. 이때 **실패 모드도 함께 통일**할 것 — `resultSpellSnapshots`는 `?.` 없이 즉시 TypeError(시끄러운 실패, C1 교훈상 올바름)인데 `_snapshotResult`의 `DataManager.instance?.getEnemy(id)?.name`는 instance null 시 모든 킬을 조용히 드롭한다(C1이 고친 바로 그 장애 모드). | `../qa/result-stats-review-issues.md` R1·R4 (재리뷰 2026-07-10) | 낮음(미발현) |
...
| J7 | ✨🔧📐 | **메타 진행·경제 — 누적 통계 + 퀘스트/업적 보상 → 포인트 → 메타 구매(능력치·아이템·스킨)** | 결과 화면의 런 통계(생존·레벨·킬·마법·패시브)를 판을 넘겨 **누적**하고, 그 누적치로 퀘스트/업적(예: 총 킬 1000 달성, 달걀귀신 누적 킬 10000 돌파)을 판정해 **포인트**를 지급한다. 포인트로 캐릭터 기본 능력치 강화(이동속도·경험치 획득량·최대 체력 등)·아이템·스킨을 구매한다. 필요한 저장 구조: 적 종류별 누적 킬·총 킬·클리어 수, 캐릭터 기본 능력치(메타 강화분), 보유 스킨, 포인트 잔액, 퀘스트/업적 진행·완료 상태. **result-stats 슬라이스엔 인터페이스 추가 불필요** — 런당 통계는 이미 `goToResult()`에서 `GameResult` 전역으로 스냅샷되고, 특히 `killsByType`가 적 종류별 세분이라 "종류별 누적 킬"의 피드로 그대로 맞다. 미래 세이브가 매 판 종료 시 `GameResult`를 읽어 localStorage 누적치에 더하면 되므로, 소비처만 없을 뿐 연결 지점(seam)은 이미 있다(메타 강화분 → 런 시작 기본 스탯 배관은 별개 축, result-stats와 무관). **선행:** J3 세이브(localStorage) 기반이 먼저 서야 누적·포인트·구매 상태를 저장할 수 있다. 스코프가 커(경제·업적·상점·스킨) v1 포함 여부·슬라이스 분해는 office-hours에서 확정. **아직 확정 기획 아님.** 로드맵 §7 '메타'. 출처: 이 대화(2026-07-07), `sessions/2026-07-06-result-stats-plan.md` §2 OUT·§8. | office-hours 확정 |

> **J4 진행 — HUD 레이아웃·바·테마 토대(첫 분해):** `feat/hud-layout` 슬라이스가 J4의 첫 조각으로 HUD 바 승격 + 목업(`../decisions/hud-layout.html`) 기준 전체 레이아웃 + 테마/해상도 토대를 깐다(계획: `sessions/2026-07-02-hud-layout-plan.md`). 이 슬라이스가 청사진 자리만 잡고 **후속으로 남긴** 사항: ① 스킬 그리드 쿨다운 라디얼(발사 후 회색 시계형, 마법 쿨다운 데이터 배선 필요·사용자 확정 비전) ② 미니맵 실제 기능(v2/이월) ③ 보스 체력바 데이터 배선(v1 무보스, v2) ④ 분할 XP 바 룩(`/design-consultation`). **⑤ placeholder 노드 4종(미니맵·보스HP바·메뉴버튼·스킬그리드) 배치 자체를 이번 슬라이스에서 제외(2026-07-03 사용자 결정)** — HUD 바·레이아웃·테마에 집중, placeholder 룩·배치는 후속 UI 슬라이스로(hud-layout-test.md §4.2). 근거·범위는 그 계획 §2 OUT 참조.
>
> **J4 진행 — 마법 아이콘 행(P0-2, 슬롯 정원 + 티어 라벨):** `feat/spell-icon-row` 슬라이스(PR #52)가 위 후속 ①의 "스킬 그리드" 자리에 실제 슬롯 정원을 채운다. HUD 우하단에 `LoadoutLogic.MAX_SLOTS`개 슬롯 프레임을 3×2 그리드로 상시 표시하고, 보유 마법을 티어 오름차순으로 분류색 placeholder + 티어 라벨(F1·I2)로 채운다. 슬롯 빌드(정렬·라벨·빈칸 패딩)는 순수 로직 `SpellIconRowLogic.buildSpellIconRow`로 뽑아 `HudController`는 노드 매핑만 한다. `MAX_SLOTS`만 바꾸면 표시 칸이 따라 변하도록 슬롯을 런타임 인스턴스화(에디터 무변경). 계획: `sessions/2026-07-05-spell-icon-row-plan.md`. 이 슬라이스가 **후속으로 남긴** 사항: ⓐ 호버 툴팁 + 강화 브레이크다운(개별/분류/전역) — 일시정지 메뉴(P0-3) 후보, ⓑ 쿨다운 라디얼(= 위 ①, 여전히 열림), ⓒ 최종 마법 아이콘 아트(아트 단계 스프라이트 교체). 근거·범위는 그 계획 §2 OUT·§8 참조.
>
> **J4 진행 — 일시정지 메뉴(P0-3):** `feat/pause-menu` 슬라이스(PR #53)가 ESC로 여는 수동 일시정지를 붙인다. `GameState.Paused` 신설 + `GameManager.enterPause`/`resumePause`(기존 `enterLevelUp` 쌍 미러 — 소프트 일시정지라 기존 `state!==Playing` 가드가 전장을 얼림), ESC 토글 결정은 순수 함수 `PauseMenuLogic.pauseToggleAction`(카드 선택 중 ESC 무시). 모달 3버튼(Resume·재시작·메뉴)은 기존 `restart()`/`goToMenu()` 재사용. 라벨은 `PauseController`가 `t()`로 채우는 코드 구동 i18n(main 씬 나머지와 일관). **함께 닫은 백로그:** I2(일시정지 중 XP 흡수 차단 — `XPItemController` 가드), I3(정지 발사체가 UI 위로 렌더 — 라이브 검증 결과 현 씬 구성이 이미 정상, 미재현). 계획: `sessions/2026-07-06-pause-menu-plan.md`. **후속으로 남긴** 사항: ⓐ 설정 화면(P1-6, 오디오 J6 선행), ⓑ 호버 툴팁 + 강화 브레이크다운(위 spell-icon-row ⓐ와 동일 후보), ⓒ **`PauseController.titleLabel` @property 미연결** — 타이틀이 정적 텍스트라 언어 전환 시 안 바뀜(버튼은 코드 구동으로 바뀜). `Title` 노드를 `titleLabel`에 연결하면 닫힘(코드 무변경) — 인게임 언어 토글(P1-6)이 생길 때 함께 처리. 근거·범위는 그 계획 §2 OUT·§8, `../qa/pause-menu-review-issues.md` 참조.
