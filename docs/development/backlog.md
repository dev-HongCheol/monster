...
| F25 | 🐛📐 | **근접 휘두르기 마커 반각 클램프(89°) vs `coneHitsTarget` 무제한 각 비대칭** — 마커는 `tan(90°)` 발산을 막으려 반각을 89°로 클램프하는데 명중 판정(`coneHitsTarget`)은 각을 클램프하지 않는다. `coneAngleDeg ≥ ~178°` 데이터가 생기면 마커가 실제 명중 부채꼴보다 좁게 그려져 "예고보다 넓게 맞는" 공정성 버그가 된다. **현재 미발현**(실 데이터 반각 75°/60°/45° ≪ 89°). 닫을 때: `coneHitsTarget`에 "무제한 각 허용(판정이 정본, 마커는 렌더용 근사)" 주석 + 데이터 테스트에 `coneAngleDeg ≤ 178` 상한 단언으로 마커·판정을 실 데이터에서 증명적 동기화. | `../qa/enemy-melee-sweep-review-issues.md` M1 (S3 코드 리뷰 2026-07-01) | 낮음(미발현) |
