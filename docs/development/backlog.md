...
| F26 | 🎨 | HP 라벨은 천단위 콤마, XP 라벨은 미적용 — 후반 숫자 표기 불일치 | `_updateHp`는 `formatNumber`로 HP를 `1,205`로 찍지만 `_updateXpInfo`의 `hud.xp` `cur`/`req`는 raw라 바로 아래 XP가 `1205`로 보인다. XP 스케일이 4자리 이상 커지는 시점에 두 수치 라벨 포맷을 `formatNumber`로 수렴. 현재 XP는 대체로 작아 저노출. | `../qa/hud-layout-review-issues.md` R2 (rework 재리뷰 2026-07-03) | 낮음 |
