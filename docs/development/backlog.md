...
| F16 | ♻️ | 그리드 질의 → 타겟 수집 루프 중복 (`SpellCaster._castNova` ↔ `Projectile._detonate`) | 두 곳이 같은 "반경으로 그리드 질의 → `ExplosionTarget[]` + `EnemyController[]` 병렬 수집" 블록이라 `ExplosionTarget` 필드 변경 시 두 군데를 고쳐야 함. `collectExplosionTargets(cx,cy,r)→{targets,ctrls}` 공유 헬퍼로 추출 가능. frost-nova 디스패치 옵션 B의 의도된 트레이드오프. **계획(feat/inferno, 2026-06-24):** 세 번째 호출부(인페르노 궤도 충돌 `_applyOrbHit`)가 생겨 rule-of-three 성립 → `collectTargetsInRadius(cx,cy,r)→{targets,ctrls}`로 수집 서브루프만 추출 예정(`_castNova`·`_detonate`·`_applyOrbHit` 공유). dedup 집합 수명 통합(전체 추출)은 §12.1 출력 일반화와 함께 잔여. | `../qa/frost-nova-review-issues.md` M-1, `../planning/magic-system-mage.md` §12.1 | 낮음 |
