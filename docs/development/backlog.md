...
| F14 | 📐 | CC 단일 슬롯 cross-strength 합치기 의미 — 약한·긴 소스가 강한 강도의 지속을 연장 | `applyControl`이 강도·지속을 각각 max로 합쳐, 강도 낮고 지속 긴 소스(예: 긴 슬로우)가 이미 걸린 강한 강도(빙결)의 남은 시간을 늘린다. 이번 슬라이스는 Stun만 생산해 미발현. 슬로우(S3)·빙결(S6)이 들어와 강도가 섞이기 시작하면 "강도별 소스를 따로 추적할지(per-source) vs 현 max/max 유지"를 결정. 테스트 `MagicCc.test.ts` cross-strength 케이스에 현 동작이 의도임을 주석으로 고정해 둠. | `../qa/magic-cc-review-issues.md` #2 | 낮음 |
