...
색인은 특정 커밋 기준으로 굳는다(이 문서 작성 시점은 `2c117af6`). **낡은 색인은 없는 것보다 나쁘다** — 이미 고친 코드를 근거로 답하게 되기 때문이다.

슬라이스를 머지한 뒤 아래를 돌린다. 증분이라 처음보다 훨씬 빠르다.

```bash
cd ~/gbrain
bun src/cli.ts sync --source monster
...
- **`/sync-gbrain` 상시 운용과 `CLAUDE.md` 라우팅 블록** — 실제 작업에서 얼마나 쓰이는지 겪어 본 뒤 붙인다. `CLAUDE.md`는 매 세션 전량 로드되므로 내용을 늘리는 결정은 실사용 근거가 쌓인 뒤에 하는 편이 낫다.
