...
- **작성일:** 2026-07-15
- **상태:** 완료 — 7단계 사용자 QA 통과, PR #59 (2026-07-16)
...
**D2(스키마 방어) 편입 범위** — 이번엔 전면 스키마 검증(D2 전체)을 하지 않고, 이 필드에 대한 **국소 방어**만 넣는다. `regions`가 없으면 무해저드(안전 기본값). 정점이 3개 미만인 폴리곤은 면적이 없어 판정이 무의미하므로 건너뛰고 한 번 경고한다. `playerSpeedMul`이 누락되면 의도치 않은 감속을 조용히 적용하지 않도록 **1.0(효과 없음)으로 폴백하고 경고**한다(의도적 0.5와 누락을 구분 — D2의 핵심 취지). **정점이 아레나 경계 밖(`|x| > size.x/2` 또는 `|y| > size.y/2`)이면 한 번 경고한다** — 폴리곤 좌표는 `size`와 같은 원점 중심 px 공간이라 둘은 함께 맞춰야 하는 한 단위인데(폴리곤 x 양끝 ±1200이 지금 `size/2`에 손으로 맞춰져 강이 좌우 강안에 닿는다), `size`만 키우고 폴리곤을 안 고치면 강이 새 경계에 못 닿고 맵 한복판에 뜨기 때문이다. 이 방어들이 어긋나면 나타나는 증상: 방어가 없으면 오타 하나가 강 전체를 감속 0으로 만들거나(무효과) 잘못된 배율로 조용히 굴러가거나, 강이 소리 없이 맵 가운데로 떠서 7단계에서 원인을 눈으로만 찾게 된다.
...
MapManager의 검증 로직(정점<3 건너뜀, `playerSpeedMul` 폴백, 정점 아레나 밖 경고)은 Cocos 의존이라 순수 테스트가 아니며, QA 문서의 수동 항목(임시 데이터 조작)으로 검증한다.
...
4. **물 구역 표현 — 폴리곤 유지 (대안: 마스크/그리드, F41에서 재검토)**. 승인 전 사용자와 authoring 방식·시각 각짐을 검토했다(2026-07-16). 마스크(강을 이미지로 칠해 저해상도 불리언 그리드로 구움)는 세 가지 이점이 있다 — (a) 정점 손 authoring을 없애 "곡선을 직선 조각으로 트레이싱"하는 어색함이 사라지고, (b) 그리드가 아레나 전체를 덮도록 정규화돼 있어 §4.2가 방어하는 `size` 결합까지 구조적으로 풀리며, (c) F41 아트의 강 레이어에서 마스크를 굽는 경로로 수렴해 최종엔 authoring 자체가 없어진다. 그럼에도 **이번엔 계획대로 폴리곤으로 진행**한다(사용자 결정, "일단 그대로"). 근거: 폴리곤은 정점이 `seoul.json` 데이터라 재조정이 싸고, 마스크 전환은 데이터 스키마·순수 함수(`pointInPolygon`→`isWaterAt`)·렌더를 통째로 갈아끼우는 별도 작업이라 최종 아트가 들어오는 F41에서 함께 하는 게 총 공수가 작다. 각짐은 게임 1:1 줌에서 "그리드 셀 크기 = 화면 계단"이며(256²≈9px로 유기적 수면선에선 거의 안 보임), F41 후엔 마스크가 화면에 안 그려져 각짐이 시각에서 사라진다는 것을 그 논의에서 만든 비교 도구로 확인했다. → 백로그 `F52`(구 `F48` — ID 충돌로 2026-07-17 재번호).
