...
- **상태:** ✅ 완료 (PR #47). 전체 스위트 344/344 GREEN, 코드 리뷰 Ready to merge — Yes, 7단계 인게임 테스트 통과(발사체 정지·무피해·재개 정상). 7단계에서 발견한 렌더 레이어 이슈(정지 발사체가 카드 글씨를 가림)는 범위 밖 후속으로 백로그 I3에 분리.
