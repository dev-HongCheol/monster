...
**Player의 `UITransform`은 48×96 그대로 둔다.** `PlayerController`의 `_halfHeight`가 이 값을 읽어 발치 오프셋을 유도하는데(`PlayerController.ts:83`), 이 값은 Player 자기 노드에서 오고 Cocos는 자식 크기를 부모 `UITransform`에 전파하지 않는다. 그래서 시각 층이 아무리 커져도 발치 · 피격 박스 · 이동 충돌이 안 바뀐다.

시각 자식의 Content Size는 공통 배율 `K`로 정한다. 가로와 세로에 다른 값을 쓰지 않는다.

```
K = 게임 표시 높이 / 기준 몸 소스 높이 = 96 / 493 = 0.1947262

Body 자식        246K × 493K = 47.903 × 96.000
Weapon 자식(600×701) = 116.836 × 136.503
```

`K`로 통일하면 몸 층이 47.903 폭이 되어, 지금까지 캔버스를 48×96에 눌러 넣으며 생긴 가로 0.20% 늘어남이 사라진다. 표시 크기 48px에서 0.1px이라 화면에서는 드러나지 않는다. 근거와 계약 전문은 [ADR 009](../../decisions/009-visual-bounds-exceed-body.md)가 든다.
