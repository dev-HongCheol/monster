...
현재는 `Vec3.lengthSqr() > 1`을 검사하고 `Vec3.normalize()`를 부른다. 순수 모듈은 cc를 못 쓰므로 같은 결과를 직접 계산한다 — 두 축이 모두 0이 아니면(=대각선) 각 성분에 `Math.SQRT1_2`를 곱한다. 조건 `lengthSqr > 1`이 성립하는 경우는 대각선뿐이라(단일 축이면 1, 무입력이면 0) 동작은 같다.
