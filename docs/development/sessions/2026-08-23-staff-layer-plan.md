...
- **작성일:** 2026-08-23 (같은 날 계획 리뷰 반영 개정 · 범위 축소 개정 · 정렬 기준 개정) · 2026-08-24 개정 (생성 회차 결과 · 사용자 검증 결과 · 양손 장비 결정)
- **브랜치:** `feat/staff-layer`
- **상태:** 진행 중 — 7단계 사용자 검증 (Draft PR #91)
...
- **넘긴 것:** 지팡이 노드 배선과 파츠 컷 → 리깅 슬라이스(`F59` ①) · v2 양손 장비 정책과 층 표 분할 → `F103` · 보조 손 재생성 → `F104` · `hurtboxHalfWidth` 반영 → `F66` ③ · `F102` ①③④
...
**넷 중 셋은 시트를 다시 뽑아야 닫힌다.** 코드가 한 일은 다음 회차가 같은 자리에서 걸리게 만드는 것뿐이고(생성 판정 두 줄과 출하 관문 둘), 지금 열두 장은 그대로 나간다(§11).
...
**한 자리는 일부러 비워 둔다.** 실행 지침 §8.2.2의 편집 지시문 셋에는 이 손 요구가 안 들어간다. 지시문은 턴어라운드에서 받은 손을 **유지**시키는 자리라 §7.1이 먼저 서야 문장이 정해지므로, `F104` ⑤가 다음 회차에서 든다.
...
- **보조 손과 팔 앞뒤의 재생성(2026-08-24 결정).** 팔의 앞뒤와 손 모양은 자세라 편집으로 못 바꾸므로(실행 지침 §2.0) 4방향 턴어라운드부터 다시 돌아야 하고, 세 시트를 한 세션에서 받아야 해서(§5) 부분 재생성이 없다 — §6.1의 결함 셋도 같은 회차에서 함께 닫힌다. **그래도 이 슬라이스는 하지 않는다** — 정렬 함수 손질과 출하 실행기는 손 위치와 무관하게 지금 들어가야 하고, 현행 열두 장이 이미 구 파이프라인 산물(지팡이 잘라낸 자국·뭉친 손)보다 낫기 때문이다. 재생성은 `F104`가 든다.
