...
- **작성일:** 2026-08-23 (같은 날 계획 리뷰 반영 개정 · 범위 축소 개정 · 정렬 기준 개정)
- **브랜치:** `feat/staff-layer`
- **상태:** 진행 중 — 계획 §1~§11은 2026-08-23 시점 그대로 두고, 그 뒤에 바뀐 것은 **§12**가 든다
- **정본:** [`art-generation-playbook.md`](../../design/spec/art-generation-playbook.md) §8.2 — 생성 판정 항목에 손 조건 셋이 붙고, 지팡이를 지우는 편집 지시문 셋이 새로 들어간다. 같은 문서 §8.7과 [`art-asset-spec.md`](../../design/spec/art-asset-spec.md) §3.2·§3.3도 함께 바뀐다. 트림 상자로 설명하던 인과가 Sprite Trim을 끄면서 정렬 기준으로 옮겨 갔고, 그 정렬 기준 자체도 이번에 바뀌기 때문이다(§4.3). **이 목록은 §12.5가 셋 늘린다.**
- **닫는 백로그:** `F102` ②⑤⑥(`footCenterX`의 이름과 동작, 매팅 캐시의 자리, 호출부 없는 산출물 셋). **`F67`은 생성·교체 축만 닫고 배선 축은 열어 둔다** — 배선을 이번에 안 하기로 했다(2026-08-23 사용자 결정, §1)
- **넘긴 것:** 지팡이 노드 배선과 파츠 컷 → 리깅 슬라이스(`F59` ①) · v2 무기별 쥔 손 정책 → 신규 백로그 항목 · `hurtboxHalfWidth` 반영 → `F66` ③ · `player_mage_bridge.png` 정리 → `F101` · `F102` ①③④
...
확정된 시트 셋을 각각 입력에 넣고 **지팡이만 지운 변형본**을 받는다. 실행 지침 §7.3이 A에서 B(삭발), B에서 C(맨살)로 가리는 것을 하나씩 걷어낸 것과 같은 수법이고, 지팡이가 그 셋 중 마지막 하나다.
...
- **`player_mage_bridge.png` 삭제.** 참조가 0건인 것은 확인됐지만, 지우기 전에 Cocos가 참조 없는 자산을 빌드에 싣는지부터 봐야 한다는 조건이 `F101`에 붙어 있다.
...
## 12. 계획 뒤에 바뀐 것 (2026-08-24)

계획을 세운 뒤 실제로 뽑고 사용자가 검증하면서 셋이 바뀌었다. **앞 절들은 2026-08-23 시점의 기록이라 고치지 않고**, 지금 무엇이 서 있는지를 여기 모은다.

### 12.1 생성 방식이 갈아탔다

§5는 세 시트를 각각 자기 원본에서 편집해 받을 작정이었는데, 그렇게 받은 옷 입은 판이 네 방향 인물 세로 편차 24px로 떨어졌다. 삭발 판 하나를 축으로 삼고 나머지 둘을 거기서 파생시키는 방식으로 갈아타 3px로 내려왔다. 회차 전문은 [`2026-08-24-staff-sheet-generation.md`](2026-08-24-staff-sheet-generation.md)가 든다.

### 12.2 사용자 검증이 결함 넷을 찾았다

눈 흰자가 통째로 투명(두 장) · 뒷모습의 쥔 손이 손가락을 편 손으로 바뀜(세 시트 전부) · 맨살 판 왼쪽에서 지팡이 쥐던 팔이 어깨부터 누락 · 맨살 시트만 인물이 6% 작게 그려짐. **넷 다 §8의 자동 검증과 §2.2의 규격 실측을 통과한 상태였다** — 어긋난 것은 그 판정들이 안 보는 자리였고, 그래서 넷 다 사람이 눈으로 찾았다. 실측·막는 방법·아직 안 고친 것은 [`../../qa/staff-layer-test.md`](../../qa/staff-layer-test.md) §7이 들며 여기 되풀이하지 않는다.

**넷 중 셋은 시트를 다시 뽑아야 닫힌다.** 코드가 한 일은 다음 회차가 같은 자리에서 걸리게 만드는 것뿐이고, 지금 열두 장은 그대로 나간다.

### 12.3 양손 장비가 정해졌고, 그래서 손 위치 요구가 바뀌었다

§6은 v2 쥔 손 정책을 「손 모양을 고정하고 무기가 그 위를 지난다」로 잡고 확정을 리깅 슬라이스로 넘겼다. 그 위에 2026-08-24 사용자 결정 넷이 얹혔다.

- **보조 손에도 장비를 들린다**(방패·책).
- **그 보조장비를 손을 가리는 방패 형태로 통일한다** — 다른 캐릭터에도 범용으로 쓰려는 것이다. 방패가 손을 통째로 덮으므로 그 밑의 손 모양은 화면에 안 나오고, §6이 남긴 「손가락 일부만 무기 앞으로 오는 자리가 없다」 미결이 보조 쪽에서는 닫힌다. **남는 것은 주무기 손 하나다.**
...
### 12.4 그래서 재생성은 2단계부터 돈다

팔의 앞뒤와 손 모양은 자세라서 편집으로 바꿀 수 없다(실행 지침 §2.0 — 한 번도 생성된 적 없는 부위는 편집으로 발명되지 않는다). 그래서 다음 회차는 §5가 상정한 편집 변형본이 아니라 **4방향 턴어라운드부터** 다시 돌고, 거기서 받은 삭발 판을 축으로 베이스 시트 둘과 지팡이 없는 변형 셋이 따라온다. **12.2의 결함 셋도 같은 회차에서 함께 닫힌다** — 세 시트를 한 세션에서 받아야 하므로(§5) 부분 재생성이 없고, 어차피 한 번에 도는 김에 함께 잡는 편이 싸다.

**이 슬라이스는 재생성을 하지 않는다.** 정렬 함수 손질과 출하 실행기는 손 위치와 무관하게 지금 들어가야 하고, 현행 열두 장이 이미 구 파이프라인 산물(지팡이 잘라낸 자국·뭉친 손)보다 낫기 때문이다. 재생성은 백로그 `F104`가 든다.

### 12.5 정본과 백로그를 어디까지 고쳤나

| 문서 | 무엇이 들어갔나 |
|---|---|
| 실행 지침 §7.1·§7.2·§8.2.1 | 측면 두 장에서 쥔 팔이 화면 가까운 쪽이라는 프롬프트 한 줄과 판정, 빈 손을 살짝 오므린 모양으로 받는다는 프롬프트 한 줄과 판정 |
| 실행 지침 §8.8 | 보조 손에 들릴 방패도 판정 밖이라는 것과, 층 표 미결의 절반이 닫힌 것 |
| [판정 규칙](../spec/game-combat.md) §3 | 방패도 「장비는 외형만 바꾼다」의 예외가 아니라는 명시 |
| 백로그 `F103` | 양손 장비 정책 — 팔 한 벌 단위 이동, 역할 기준 슬롯, 주무기 손만 앞손가락 조각, Draw Order의 Spine Essential 포함 여부 확인 |
| 백로그 `F104` | 신규 — 보조 손 재생성. 이유는 v1 화면 하나이고, 착수는 이 슬라이스 머지 뒤다 |

**남은 것이 하나 있다.** 실행 지침 §8.2.2의 편집 지시문 셋에는 손 요구가 아직 안 박혀 있다. 지시문은 턴어라운드에서 받은 손을 **유지**시키는 자리라 §7.1이 먼저 서야 문장이 정해지므로, `F104` ⑤가 다음 회차에서 든다.

### 12.6 `player_mage_bridge.png`을 지웠다

§11은 이 삭제를 이번에 안 하기로 적어 뒀다. `F101`이 「지우기 전에 Cocos가 참조 없는 자산을 빌드에 싣는지부터 확인한다」는 조건을 걸었기 때문인데, **그 조건이 재던 것은 안전이 아니라 우선순위다** — 안 실으면 용량 문제가 아니라 정리 문제일 뿐이라 뒤로 미룰 근거가 된다는 뜻이었다. 2026-08-24에 사용자가 이번 PR에 넣기로 정했으므로 그 저울질은 답이 났고, 남는 것은 안전 확인 하나다.
...
**`F101`은 이 슬라이스가 닫는다.** 아카이브로 옮기는 것은 `pr-ready`가 한다.
