...
- **전역 i-frame + 틱당 max.** 플레이어는 고정 주기 `T`(피격 틱 = 무적 구간)마다 피해를 **한 번**만 받는다. 그 한 번의 값은 그 틱 동안 들어온 모든 피해원(접촉·발사체·돌진·휘두르기) 중 **가장 센 것**이다. 부채꼴 N발이 동시에 닿아도, 여러 적이 동시에 때려도 그 틱엔 가장 위험한 한 방만 들어온다.
