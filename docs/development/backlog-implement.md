...
| F9 | 열림 | 낮음 | 🔧 씬 카메라 크로스머신 churn | MacBook 작업 / Windows 테스트 시 `main.scene` Camera 노드의 `_lpos.y`·`_orthoHeight`가 재계산돼 무관한 diff가 난다(장비별 해상도 차이로 에디터가 카메라를 재fit·재직렬화). 매 PR 테스트마다 반복된다. 커밋 전 `git diff *.scene`으로 거르거나 카메라 fit 정책·고정값으로 안정화. | projectile-count 테스트 (2026-06-11) |
