...
| F101 | 진행중(`feat/staff-layer`) | 낮음 | 🧹 씬·프리팹이 안 쓰는 아트 자산 정리 | `player_mage_bridge.png`가 어느 씬·프리팹에서도 참조되지 않는다(UUID 역참조로 확인, 2026-08-20). 4방향 스프라이트로 교체되면서 남은 브릿지 자산이다. `player_staff.png`도 같은 상태지만 이쪽은 **배선 대기**라 성격이 다르다(지팡이 노드 배선이 별도 슬라이스로 남아 있다 — 실행 지침 §8.8). 지우기 전에 Cocos가 참조 없는 자산을 빌드에 싣는지부터 확인한다 — 안 실으면 용량 문제가 아니라 정리 문제일 뿐이라 우선순위가 더 내려간다. **`feat/staff-layer`가 `player_mage_bridge.png`을 지워 닫는다**(2026-08-24 사용자 결정) — 그 조건이 재던 것은 안전이 아니라 우선순위였고, 삭제 직전에 UUID 역참조가 0건인 것을 다시 확인했다([`sessions/2026-08-23-staff-layer-plan.md`](sessions/2026-08-23-staff-layer-plan.md) §12.6). `player_staff.png`는 배선 대기라 남는다. | [`sessions/2026-08-20-license-audit-plan.md`](sessions/2026-08-20-license-audit-plan.md) §5 |
| F102 | 열림 | 낮음 | 🧹 아트 파이프라인 도구의 잔여 정리 | `feat/ai-matting` 코드리뷰가 낸 Minor 중 안 고치고 남긴 것들이다. **②⑤⑥은 `feat/staff-layer`가 닫았다** — ②는 `footCenterX`를 발만 보는 `footBand`로 다시 쓰면서 알파 16 이하를 없는 것으로 보게 해 이름과 동작을 맞췄고, ⑤는 매팅 캐시를 `docs/temp/`에서 `art-source/matting-cache/`로 옮겼으며(앞 폴더의 규칙이 「언제든 지워도 되는 스크래치」인데 이 캐시는 지우면 돈이 든다), ⑥은 셋 다 호출부를 얻었다(`readPngSize`는 출하본 캔버스 검사, `IMatteResult.cachePath`는 `build.ts`의 정렬 실패 메시지, `achromaticRatio`는 `judge.ts` 출력). 남은 것은 셋이다. **①은 전제가 절반 틀렸다** — `cropColumns`는 음수 구간과 범위 초과를 이미 던지고, 실제로 새는 것은 **비정수 구간**과 **음수 `margin`**이다. `build.ts`가 `panelColumns` 결과를 그대로 넘기므로 그마저 안 걸린다. ③ `minAlpha`가 함수마다 포함(`>=`)·제외(`>`)로 갈린다 — 한쪽으로 통일하고 이유를 적는다. 여기에 같은 성격의 인자 정합 하나를 붙인다 — `IPanelColumnsOptions`가 `rowBackground`를 켜면 필수 필드 `background`를 안 보는데, 지금은 주석만 그 사실을 막고 있어 판별 유니온으로 바꾸면 타입이 막는다(`feat/staff-layer` 리뷰 M-안 고친 것). ④ `alignToCanvas`가 `trimBox`·`footBand`로 같은 이미지를 두 번 훑는다 — 12장 규모에서는 문제가 아니라 미룬다. | [`../qa/ai-matting-review-issues.md`](../qa/ai-matting-review-issues.md) M6~M10·덧 · [`../qa/staff-layer-test.md`](../qa/staff-layer-test.md) |
