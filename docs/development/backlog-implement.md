...
| F55 | 열림 | 낮음 | 🐛🔧 장애물 최소 변 하한 미강제 — 맵 확대로 서브스텝 관통 마진이 절반이 됐다 | 미발현 — `resolveCircleMove`의 서브스텝 상한(`MAX_SUBSTEPS=64`)이 무는 극단에서 서브스텝 길이 = 이동거리/64다. 맵이 4800으로 커지며 대각선이 6788px가 되어 그 상한 서브스텝이 2400 시절 ~53px에서 106px로 약 2배가 됐다. 장애물 확장 두께 하한(최소 변 120px + 2×최소반지름 18 = 156px) 대비 여유가 ~103px에서 ~50px로 줄었다. 여전히 106 < 156이라 관통은 없지만, 이 안전은 **"장애물 최소 변 120px" 전제에 의존하고 그 하한을 코드가 강제하지 않는다**(`MapManager` 색인은 크기 0·한 변 300px 초과만 경고, 최소 변 하한 경고 없음). 한 변 ~70px 미만 박스를 극단 dt에서 놓으면 이론상 관통. **고칠 곳은 F50과 같은 데이터 경계 배터리** — `MapManager` 색인에 최소 변 하한 경고 한 줄을 붙이면 얇은 박스가 조용히 관통 위험을 만드는 것을 막는다. **함께:** [[F50]]·[[D2]] 스키마 검증에 흡수 가능. | `../qa/map-size-2x-review-issues.md` M-2 |
...
| F54 | 열림 | 낮음 | ♻️ "표준 서울 아레나 2400" 테스트 픽스처 라벨이 실제 맵(4800)과 어긋남 | `tests/logic/MapArena.test.ts:20-21`·`SpawnGeometry.test.ts:36-37`이 `ARENA = {2400, 2400}`를 "표준 서울 아레나(placeholder 크기)"로 라벨링한다. map-size-2x로 실제 서울이 4800이 되며 이 고유명 라벨이 사실과 어긋났다 — 테스트 로직은 크기 불가지라 통과 무영향, 순수 가독성. 라벨을 "합성 아레나(크기 불가지)" 류로 중립화하거나 실제 seoul=4800임을 한 줄 덧붙인다(값 2400은 임의 픽스처라 불변). map-size-2x가 무관 파일 수정 규칙으로 손대지 않았다. | `../qa/map-size-2x-review-issues.md` M-1 |
