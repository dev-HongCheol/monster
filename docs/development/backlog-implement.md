...
| F53 | 열림 | 중 | 📐🔧 볼록 프리미티브 충돌 — 원(먼저)·캡슐·OBB | 장애물 충돌이 축정렬 사각형(AABB) 전용이라 원형 건물(동대문 체육관 돔 등)을 감싸면 대각선에서 `~0.41R`(R=200px면 ~83px) 투명벽이 생겨 "닿지도 않았는데 막힘" → 맵 다형성이 죽는다. 이동 주체가 이미 원이라 원 프리미티브가 최저비용 — 형태 seam(`rect|circle` 태그 유니온) 도입 + **원만** 먼저 구현(Approach A), OBB(대각선 버스)·캡슐(경기장)은 같은 seam 위 후속. 밀어내기(`resolveCircleMove`)는 최근접점 일반화로 값싸고, **진짜 비용은 형태별 우회 스티어링(`steerAroundObstacles`, 원=접선)**. 가까운 별개 잔해 사이 통과(좁은 틈) 튜닝·`MAX_RESOLVE_PASSES` 검증 포함. **함께:** [[F50]] 좌표 NaN 가드를 이 색인에 흡수. 착수는 `feat/building-collision` 머지 후. | `sessions/2026-07-18-convex-primitive-collision-design.md`, `sessions/2026-07-14-map-space-roadmap.md` D2·D4 |
...
| F43 | 진행중 (feat/building-collision) | 중 | 📐🔧 적 경로탐색 — 장애물 회피 | 지금 적은 직선 추적(`_followPlayer`)이라 장애물을 못 피한다. **2026-07-14 결정으로 범위가 크게 줄었다** — 한강이 배리어가 아니라 소프트 해저드가 되면서(F39) **적이 큰 지형을 우회할 일이 없어졌고, 흐름장(flow field) 경로탐색·영역 연결성 검사가 통째로 불필요해졌다.** 남은 것은 건물(F38) 하나를 도는 **로컬 회피**뿐이다: 이동 결과를 표면 밖으로 밀어내고 벽면 접선으로 미끄러뜨린다(collide-and-slide + 코너 바이어스). ~~방향 계산을 바꾸지 않고 `setPosition` 직전에 후처리 한 단계로 붙이므로 `MovementLogic` 시그니처는 그대로다(스티어링과 내비게이션 분리).~~ → **정정 (2026-07-17):** 방향 계산을 안 바꾸는 것은 **불가능**했다. 밀어내기는 속도의 법선 성분만 지우므로 정면 진입에서 변위가 0이 되고, 남는 접선 성분이 적을 정면 지점 쪽으로 미끄러뜨려 정지점이 **끌개**가 된다 — 비스듬히 들어온 적까지 빨려들어 멎는다. 해소는 못 갈 곳을 막을 뿐 **새 방향을 만들지 못한다**. 이 항목이 처음부터 적어 둔 「**코너 바이어스**」가 바로 그 빠진 조각인데, `feat/building-collision` 계획이 §2.3에서 그것을 떨어뜨리고 collide-and-slide만 남겼다가 7단계에서 "적이 벽에 붙은 채 굳음"으로 터져 리워크했다. 실제 구현은 **우회 스티어링(`steerAroundObstacles`) + 밀어내기 해소** 2단계다. `MovementLogic` 시그니처가 그대로인 것은 사실이나(우회는 그 결과를 받아 덧씌우는 별도 단계), "스티어링과 내비게이션 분리"는 **성립하지 않는다** — 스티어링이 장애물을 알아야 한다. **D4(2026-07-14)가 이 축소를 떠받친다** — 「방」을 만들지 않고 장애물을 작은 볼록 사각형(한 변 ≤ 300px)으로 묶었으므로 로컬 회피만으로 충분하다. 방·복도가 도입되면 로컬 회피로는 부족해지고, `feat/spawn-geometry`의 유클리드 교전 카운트까지 도달성 기반으로 갈아엎어야 한다. | `sessions/2026-07-14-map-space-roadmap.md` §4·D4, `sessions/2026-07-11-map-arena-plan.md` §7 |
| F50 | 열림 | 낮음 | 🐛🔧 비유한 장애물 좌표가 이동 주체 위치를 NaN으로 오염시킨다 | 미발현 — `resolveCircleMove`는 `from`·`to`·`radius`의 NaN은 막지만 **`obstacles` 항목 자체는 안 본다**. `cx`나 `halfW`가 NaN이면 `d2 >= r2`가 false로 새고(NaN 비교) 내부 탈출 분기로 빠져 위치가 NaN이 된다 — 그 뒤 모든 프레임의 이동·충돌·렌더가 함께 무너지는데, 바로 그 실패 모드를 막으려고 `to`에는 이미 가드가 있다(가드가 반쪽인 셈). 우회 스티어링(`steerAroundObstacles`) 쪽은 오염 5종 전부에 안전함을 확인했다. **왜 미발현인가:** 값의 출처가 Cocos `UITransform.contentSize`·노드 좌표라 에디터로는 NaN을 못 넣는다(엔진 손상·씬 파일 수동 편집이라야 도달). 공격면 아님 — 신뢰 경계 밖 입력이 없는 로컬 싱글플레이. **고칠 곳은 D2 원칙대로 데이터 경계인 `MapManager` 색인** — 이미 크기 0·scale·angle·300px·경계 밖·간격을 경고로 거르고 있으므로 `Number.isFinite` 검사 한 줄이 그 배터리에 그대로 붙는다. **함께:** [[D2]] 스키마 검증에 흡수 가능. | `sessions/2026-07-16-building-collision-plan.md` (2026-07-17 리워크 /cso에서 발견), `../qa/building-collision-test.md` |
