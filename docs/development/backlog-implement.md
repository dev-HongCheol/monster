...
| F81 | 열림 | 중(다음 문서 슬라이스 전) | 📐🔧 `CLAUDE.md` 크기 예산의 여유가 얇다 | `feat/canon-spec-move`가 라우팅 표를 접으면 예산을 되찾을 것으로 계획했는데 **반대로 줄었다** — 접기가 번 약 180자를, 정본 경로마다 붙은 `spec/`과 새 행 둘이 도로 먹었다. **F69가 아트 3행을 접어 일부 되찾았다**(2026-08-14 기준 13,821 / 14,000, 212 / 240줄 — 여유 179자). 그래서 선택지 ①(아직 접지 않은 행을 더 접는다)은 **소진됐고** 남은 것은 둘이다 — ② 본문 절 하나를 `spec/`이나 절차 문서로 내보낸다, ③ 예산 수치를 올린다. **③은 그 테스트가 막으려던 재성장을 허용하는 것이므로 사람이 정한다.** | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) R1 |
| F82 | 열림 | 낮음 | ♻️🔧 링크 검사기의 남은 구멍 넷 | 2차 코드 리뷰가 낸 저순위 발견이다. ① 「표시 텍스트 = 파일명」 단언만 `blankFences`·`EXTERNAL`을 안 거쳐 코드 펜스 속 예시와 외부 URL을 오탐할 표면이 있다(지금 위반 0건이라 초록). ② `wf check-links` 가드가 대상 파일에 `findBrokenLinks` **문자열**이 있는지만 봐서, 레포 전체 회귀망 `it`만 지워도 초록을 유지한다. ③ `EMPHASIS`가 `_`를 뺀 뒤 완전 무동작인데(제목 2,747개 전수로 결과 차이 0건) 상수와 JSDoc 여섯 줄이 남아 "여기서 표기를 벗긴다"고 주장한다 — 지우고 설명을 `SLUG_KEEP`으로 옮긴다. ④ `code-conventions.md`의 표를 포인터로 줄이며 `I18n.instance.t(key, params)`라는 구체 호출 형태가 어느 정본에도 안 남았다 — `code-i18n.md` §8.1에 되살린다. ⑤ 검사 도구 쪽 중복 둘 — `DocLinks.test.ts`가 `loadDocs()`를 네 번 불러 매번 `git ls-files` + 전 문서 재읽기를 하고(`.html` 단언은 `tracked`만 필요한데 마크다운을 통째로 읽는다), `findTrackedFiles`가 이동 테스트 둘과 `loadDocs`에 세 벌 있다. 둘 다 `tests/helpers/LinkCheck.ts`가 자연스러운 자리다. | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) N2~N5 · [`../qa/art-canon-move-review-issues.md`](../qa/art-canon-move-review-issues.md) M5 |
| F84 | 열림 | 중(F75 전) | 📐 정본이 정본을 **인용**할 때를 검사가 못 본다 | 링크를 다른 정본으로 재지정하는 것은 문장이 그저 **가리킬** 때만 안전하다. 인용하거나 대상의 내용을 특징지으면 재지정이 그 주장을 거짓으로 만드는데, 링크는 멀쩡히 풀리므로 검사기도 테스트도 침묵한다. F69에서 실제로 났다 — `game-combat.md` §3을 가리키며 그 문서에 없는 문장을 인용부호로 감쌌다. 처방 둘: ① `docs-writing-style.md`에 "정본이 정본을 인용하면 대상에서 복사하고, 링크를 옮길 때는 새 대상의 해당 절을 다시 읽는다"를 넣는다, ② 정본→정본 링크 바로 뒤의 `"..."`가 대상 절에 실제로 있는지 재는 단언을 검토한다. **F75 전에 한다** — 그 슬라이스가 같은 종류의 재지정을 13곳 더 한다. | [`../qa/art-canon-move-review-issues.md`](../qa/art-canon-move-review-issues.md) C1 |
