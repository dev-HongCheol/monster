...
| F69 | 열림 | 중 | 📐🔧 아트 정본 셋을 `spec/`으로 옮기고 **결정 기록으로 가는 링크 걷어내기** | **선행 조건이 풀렸다** — 착지점이던 판정 규칙 정본 `spec/game-combat.md`가 2026-08-13에 생겼다. 대상은 `art-direction.md` 30 · `art-generation-playbook.md` 17 · `asset-production-spec.md` 12 = **59개**다(같은 문서의 `../decisions/*.html` 목업 링크 2개는 결정 기록이 아니라 남긴다). **이동도 이 슬라이스가 한다** — 셋을 `docs/design/spec/`으로 옮기고 `CLAUDE.md`의 아트 3행을 층 행으로 접는다. 앞선 슬라이스가 그 셋만 접지 못한 이유는 디자인 인덱스가 비어 있어 접으면 라우팅이 빈 표로 향하기 때문이다. 정본(`docs/design/`·`docs/planning/`)이 세션 문서·ADR을 링크하면 그 결정이 뒤집혔을 때 링크 대상이 낡은 채로 남는데, 독자는 정본을 거쳐 들어왔기 때문에 그것을 현재 명세로 읽는다(2026-08-08에 실제로 났다). 규칙은 `CLAUDE.md` 「문서 정리 규칙」에 들어가 신규 문서가 막혔고 사고가 난 머리말도 고쳤으므로, **남은 것은 기존 본문 링크 정리다.** 착수 전에 출처 §6을 본다 — 대상 목록과 처리 기준(근거 포인터는 날짜 표기로, 내용을 위임한 링크는 개별 판단), 그리고 이미 검토하고 접은 안(2단 참조 · 세션 폴더 README)이 거기 있다. | [`sessions/2026-08-08-canon-session-link-policy.md`](sessions/2026-08-08-canon-session-link-policy.md) §6 |
...
| F77 | 열림 | 중(다음 머지 전) | 🔧 gbrain 색인 갱신 절차가 실행 불가능 — `serve`와 CLI가 PGLite를 두고 다툰다 | 9단계 색인 갱신이 **2026-07-25 이후 19일간 조용히 건너뛰어졌다.** `gbrain-setup.md` §6.2가 지시하는 CLI 동기화는 상시 실행 중인 `gbrain serve`가 PGLite(단일 쓰기)를 잡고 있어 DB를 열지 못하는데, §6.2에 서버를 내리라는 말이 없다. 실패해도 아무것도 막지 않으므로 누락이 보이지 않는다. 우회로인 MCP `sync_brain`은 `--source`도 `--strategy code`도 못 받아 새 문서 31개를 `default` 소스에 넣었고(중복 사본을 한 건씩 수동 삭제해야 했다), 코드 색인은 아예 못 돌린다. 처방은 §6.2에 serve 중단·`/mcp` 재연결 단계를 넣는 것이다. **자동화는 답이 아니다** — `/sync-gbrain` 스킬도 같은 잠금을 만난다. | [`sessions/2026-08-13-gbrain-sync-lock.md`](sessions/2026-08-13-gbrain-sync-lock.md) |
...
| F80 | 열림 | 낮음 | 📐 옮긴 정본 여섯을 **맨 파일명**으로 부르는 평문 약 123곳 | `feat/canon-spec-move`가 경로가 붙은 참조는 전부 고쳤지만(D5), `conventions.md`처럼 경로 없이 이름만 적힌 언급은 그대로 뒀다. 대부분 세션·QA 문서라 그 시점의 문장으로 보존하는 것이 맞지만, 문제는 **`git grep conventions.md`가 여전히 옛 이름으로 걸린다**는 것이다. 다음 사람이 그 결과를 보고 없는 파일을 찾게 된다. 링크가 아니라서 `wf check-links`가 한 건도 못 잡는다. 고칠지 그대로 둘지를 정하는 것이 이 항목이고, 고치기로 하면 층별로 규칙을 나눠야 한다(현재 명세 층은 고치고 시점 기록은 두는 식). | [`sessions/2026-08-13-canon-spec-move-plan.md`](sessions/2026-08-13-canon-spec-move-plan.md) D5 |
| F81 | 열림 | 중(다음 문서 슬라이스 전) | 📐🔧 `CLAUDE.md` 크기 예산의 여유가 11자뿐이다 | `feat/canon-spec-move`가 라우팅 표를 접으면 예산을 되찾을 것으로 계획했는데 **반대로 줄었다**(13,936 → 13,989 / 14,000). 접기는 행 셋을 없애 약 180자를 벌었지만, 정본 경로마다 `spec/`이 붙어 다섯 군데가 늘고 `check-links` 행과 디자인 인덱스 행이 새로 붙어 그보다 더 먹었다. 여유 11자는 다음 슬라이스가 곧바로 깨뜨린다. 선택지는 셋이다 — ① 아직 접지 않은 행을 더 접는다(아트 3행은 F69가 가져간다), ② 본문 절 하나를 `spec/`이나 절차 문서로 내보낸다, ③ 예산 수치를 올린다. **③은 그 테스트가 막으려던 재성장을 허용하는 것이므로 사람이 정한다.** | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) R1 |
| F82 | 열림 | 낮음 | ♻️🔧 링크 검사기의 남은 구멍 넷 | 2차 코드 리뷰가 낸 저순위 발견이다. ① 「표시 텍스트 = 파일명」 단언만 `blankFences`·`EXTERNAL`을 안 거쳐 코드 펜스 속 예시와 외부 URL을 오탐할 표면이 있다(지금 위반 0건이라 초록). ② `wf check-links` 가드가 대상 파일에 `findBrokenLinks` **문자열**이 있는지만 봐서, 레포 전체 회귀망 `it`만 지워도 초록을 유지한다. ③ `EMPHASIS`가 `_`를 뺀 뒤 완전 무동작인데(제목 2,747개 전수로 결과 차이 0건) 상수와 JSDoc 여섯 줄이 남아 "여기서 표기를 벗긴다"고 주장한다 — 지우고 설명을 `SLUG_KEEP`으로 옮긴다. ④ `code-conventions.md`의 표를 포인터로 줄이며 `I18n.instance.t(key, params)`라는 구체 호출 형태가 어느 정본에도 안 남았다 — `code-i18n.md` §8.1에 되살린다. | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) N2~N5 |
