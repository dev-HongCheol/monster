...
| F81 | 열림 | 중(다음 문서 슬라이스 전) | 📐🔧 `CLAUDE.md` 크기 예산의 여유가 얇다 | `feat/canon-spec-move`가 라우팅 표를 접으면 예산을 되찾을 것으로 계획했는데 **반대로 줄었다** — 접기가 번 약 180자를, 정본 경로마다 붙은 `spec/`과 새 행 둘이 도로 먹었다. **F69가 아트 3행을 접어 일부 되찾았다**(2026-08-14 기준 13,821 / 14,000, 212 / 240줄 — 여유 179자). 그래서 선택지 ①(아직 접지 않은 행을 더 접는다)은 **소진됐고** 남은 것은 둘이다 — ② 본문 절 하나를 `spec/`이나 절차 문서로 내보낸다, ③ 예산 수치를 올린다. **③은 그 테스트가 막으려던 재성장을 허용하는 것이므로 사람이 정한다.** **2026-08-14 재검토(`feat/gbrain-canon`)** — ②의 유력 후보인 「에셋 `.meta` 관리 규칙」 절(1,156자, 회수 약 950자)을 넣었다가 뺐다. 그 슬라이스가 `CLAUDE.md`를 오히려 줄여 트리거가 안 당겨졌고, `.meta`는 AI가 실제로 어겨서 못박힌 **안전 규칙**이라 상주 파일에서 빼는 판단이 **F71**(절차 배달 전환의 준수율 관찰) 뒤에 와야 한다. | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) R1 |
