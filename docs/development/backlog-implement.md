...
| F73 | 열림(틀 완료) | 중(F74와 한 슬라이스) | 📐 기존 정본을 `spec/` 하위로 옮긴다 | **틀은 `feat/claude-md-split`에서 끝났다** — `docs/development/spec/`·`docs/design/spec/`과 접두사 닫힌 집합(`code`·`game`·`docs`·`ops` / `art`·`ui`), `pnpm wf canon` 생성기, 인덱스 자동 등재까지. **남은 것은 이전이다.** `architecture.md`·`conventions.md`·`i18n-guide.md`·`writing-style.md`·`glossary.md`와 `docs/design/`의 아트 정본 셋을 `git mv`로 옮기고 경로 참조를 일괄 치환한다. 절 번호는 건드리지 않는다. **같은 커밋에서 `CLAUDE.md` 라우팅 표를 층 단위로 접는다** — 지금 표는 정본을 파일 단위로 나열해서 이전만으로 경로 행 일곱 개가 한꺼번에 깨지고, 손 유지도 이미 실패했다(`architecture.md`·`i18n-guide.md`가 빠진 채 남아 ADR 002·005가 그 자리를 채웠다). 매 슬라이스 읽는 문서만 이름으로 남기고 나머지는 두 `spec/README.md`를 가리키는 행 하나로 접는다. **깨진 내부 링크를 잡을 도구가 없으므로**(`check-docs`는 phase↔문서만 본다) 마크다운 링크 검사를 함께 만든다. | [`sessions/2026-08-12-canon-layer-restructure.md`](sessions/2026-08-12-canon-layer-restructure.md) §4-1·§4-5·§4-6 |
...
| F78 | 열림 | 낮음 | ♻️🔧 `insertCanonRow`가 두 벌이다 — 각자 자기 사본을 테스트한다 | 같은 로직이 `.claude/workflow.mjs:181`(실제로 도는 것)과 `tests/helpers/CanonDoc.ts:153`(테스트가 부르는 것)에 각각 있다. 양쪽 다 테스트가 붙어 있지만 **단위 테스트는 사본을, E2E는 실물을 본다.** 그래서 한쪽만 고치면 다른 쪽이 낡은 채로 초록불을 유지하고, 어긋난 것을 알려 주는 장치가 없다. `feat/canon-spec-move`가 링크 검사기를 공유 모듈(`.claude/link-check.mjs`)로 만들면서 타입스크립트 테스트가 `.mjs`를 부르는 배관을 세우므로, **그 배관이 서면 이 항목의 비용이 크게 준다** — 그 뒤에 집는다. | [`sessions/2026-08-13-canon-spec-move-plan.md`](sessions/2026-08-13-canon-spec-move-plan.md) E4 |
