...
| F107 | 열림 | 중(검증 단계 전) | 🔧 `.gstack/` 산출물이 `pnpm check`를 빨갛게 만든다 | gstack 스킬이 `.gstack/` 아래에 남기는 JSON(`claude-available.json`·`security-reports/*.json`)이 biome 포맷 규칙에 걸려 `pnpm check`가 종료 코드 1을 낸다. **git에는 안 들어가지만**(`.gitignore:23`) 검증 단계의 `wf pass lint`가 같은 명령을 보므로 그 자리에서 막힌다. 1라운드 검증(2026-09-15)에서는 오류 0건이었고, 그 뒤 스킬이 파일을 남기면서 생겼다. 처방 후보는 biome 설정에서 `.gstack`을 검사 대상에서 빼는 것이다 — 레포 산출물이 아니라 도구 캐시라 형식을 맞출 이유가 없다. | 2026-09-16 `feat/blender-3d-gate` qa-setup 작업 중 실측 |
