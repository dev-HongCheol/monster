...
| F82 | 열림 | 낮음 | ♻️🔧 링크 검사 도구의 남은 구멍·중복 다섯 | 2차 코드 리뷰가 낸 저순위 발견이다. ① 「표시 텍스트 = 파일명」 단언만 `blankFences`·`EXTERNAL`을 안 거쳐 코드 펜스 속 예시와 외부 URL을 오탐할 표면이 있다(지금 위반 0건이라 초록). ② `wf check-links` 가드가 대상 파일에 `findBrokenLinks` **문자열**이 있는지만 봐서, 레포 전체 회귀망 `it`만 지워도 초록을 유지한다. ③ `EMPHASIS`가 `_`를 뺀 뒤 완전 무동작인데(제목 2,747개 전수로 결과 차이 0건) 상수와 JSDoc 여섯 줄이 남아 "여기서 표기를 벗긴다"고 주장한다 — 지우고 설명을 `SLUG_KEEP`으로 옮긴다. ④ `code-conventions.md`의 표를 포인터로 줄이며 `I18n.instance.t(key, params)`라는 구체 호출 형태가 어느 정본에도 안 남았다 — `code-i18n.md` §8.1에 되살린다. ⑤ 검사 도구 쪽 중복 둘 — `loadDocs()` 4회 호출과 `findTrackedFiles` 3중 복사. 둘 다 `tests/helpers/LinkCheck.ts`가 자연스러운 자리다. | [`../qa/canon-spec-move-review-issues.md`](../qa/canon-spec-move-review-issues.md) N2~N5 · [`../qa/art-canon-move-review-issues.md`](../qa/art-canon-move-review-issues.md) M5·「넘긴 것」 |
...
| F84 | 열림 | 중(F75 전) | 📐 정본이 정본을 **인용**할 때를 검사가 못 본다 | 링크 재지정은 문장이 그저 **가리킬** 때만 안전한데, 인용하면 재지정이 그 주장을 거짓으로 만들면서도 링크는 멀쩡히 풀려 검사기도 테스트도 침묵한다. F69에서 두 번 났다. **F75 전에 한다** — 그 슬라이스가 같은 종류의 재지정을 13곳 더 한다. 처방 둘(문체 규칙 · 기계 백스톱)은 출처가 든다. | [`../qa/art-canon-move-review-issues.md`](../qa/art-canon-move-review-issues.md) C1·「넘긴 것」 |
