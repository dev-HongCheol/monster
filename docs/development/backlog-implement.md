...
| F65 | 열림 | 높음 | 🐛 창 포커스를 잃으면 눌린 이동키가 고착된다 (`keyup` 유실) | **실발현·항상 재현.** 이동키를 누른 채 다른 앱 창으로 포커스를 옮겼다 돌아오면 캐릭터가 그 방향으로 계속 걷고, 반대 방향 키가 먹지 않는다(좌우는 정상). Cocos의 웹 키보드 입력(`pal/input/web/keyboard-input.ts`)이 `keydown`·`keyup`을 GameCanvas 엘리먼트에만 걸기 때문에, 포커스가 떠난 뒤 손을 뗀 `keyup`은 캔버스로 오지 않고 `PlayerController._keyUp`이 `true`로 남는다. 그 상태에서 아래키를 누르면 이동 벡터의 y가 `1 - 1 = 0`이 되어 제자리에 서므로, **반대 방향만 죽고 좌우는 멀쩡한** 증상이 된다. **`Game.EVENT_HIDE`로는 막을 수 없다** — `pal/system-info/web/system-info.ts`가 최신 브라우저에서 `visibilitychange`만 등록하고 `window`의 `blur`는 `document.hidden`이 없는 구형 브라우저용 폴백이라, 탭이 visible로 남는 창 전환에서는 발화하지 않는다(탭 전환·최소화는 발화해 `pauseByEngine`으로 게임이 멈추지만, 플래그가 남아 복귀 즉시 다시 걷는다). **배포본에도 그대로 있다** — `cc.config.json`이 web 플랫폼에 같은 `pal/input/web`을 매핑하므로 에디터 프리뷰 한정이 아니고, 실사용자에겐 Alt+Tab·알림 팝업·임베드된 iframe 밖 클릭으로 더 자주 발생한다. main에도 동일 코드가 있는 기존 버그이며(4방향 아트가 만든 회귀가 아니라 뒷모습으로 굳어 눈에 띄게 만들 뿐), 눌린 상태를 유지하는 곳은 `PlayerController` 하나다(`PauseController`는 ESC 엣지 트리거라 무관). 봉합은 포커스를 잃을 때 이동키 플래그 4개를 일괄 해제하는 것이다 — 실제 재현 경로를 덮는 것은 `window`의 `blur`(`sys.isBrowser` 가드)이고, `Game.EVENT_HIDE`는 모바일·네이티브의 백그라운드 진입용으로 함께 건다. 판단이 필요한 지점은 하나뿐인데, 지금 `game/assets/scripts/` 어디에도 DOM API를 직접 부르는 전례가 없다는 것이다. 키 상태를 순수 모듈로 빼면 "해제 후 반대 방향이 즉시 먹는다"를 회귀 테스트로 고정할 수 있다. 사용자 임시 회피책은 돌아온 뒤 같은 키를 한 번 눌렀다 떼는 것이다. | 2026-07-25 인게임 테스트 (`feat/player-4dir` 7단계). 별도 출처 문서가 없어 이 항목이 진단의 유일 기록이다 |
...
| F63 | 열림 | 낮음 | 🔧 `hitbox-viewer.html` biome 에러 4건 | repo 루트의 디버그 뷰어(player-hitbox 슬라이스 산출)가 `pnpm check`에서 에러 4 + info 29를 낸다(main부터 존재). `pnpm check --write` 전체 실행이 이것 때문에 종료코드 1을 낸다 — 슬라이스별 `pass lint`는 변경 파일만 봐서 안 걸리지만, 전체 포맷 실행이 막힌다. 뷰어를 고치거나 `biome.json` includes에서 제외한다. player-4dir(2026-07-25) 검증 중 발견. | player-4dir lint 게이트 |
