...
| F88 | 열림 | 낮음 | 📐 참조 규칙의 소급 적용 — 앵커 없는 참조 99곳·`정본:` 줄 없는 세션 62개 | 규칙은 신규부터 적용하기로 했고(`spec/docs-references.md` §13) 기계는 §7만 신규를 강제한다. 남은 소급분 셋의 규모와 세는 방법은 그 문서 §13 표가 든다. 한 슬라이스에 안 담기므로 대상 문서를 건드릴 때 함께 고치는 것이 기본이고, 일괄 작업으로 집으려면 앵커 쪽부터 — 앵커가 붙으면 제목이 바뀔 때 `wf check-links`가 빨간불을 낸다. | `sessions/2026-08-18-docs-references-plan.md` §8 |
