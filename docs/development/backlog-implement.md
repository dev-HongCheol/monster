...
| F87 | 열림 | 낮음(다음 도구 슬라이스) | 📐🔧 **자기 문서 절 인용**이 가리키는 절이 맞는지 재는 검사 | F84가 표기를 갈라 놓은 결과로 **인라인 인용에 자기 문서 귀속만 남았다**. 그러면 남은 결함은 「문서는 맞고 절이 틀린」 형태 하나인데(`playbook:713`이 §5.4의 문장을 §2.3의 것으로 인용한 실물이 있다), 이제는 절 파서를 붙여도 산문 별칭을 파일로 푸는 문제가 없어 훨씬 싸다. F84 계획이 절 단위 내용 검사를 41건 오탐으로 기각했던 이유도 대부분 사라진다 — 그때 오탐의 원인은 남의 문서 귀속이었다. **함께 볼 것 하나** — `playbook` §3이 fal.ai 모델 페이지를 인라인 인용한다. 정본→정본 밖이라 F84 범위에서 뺐는데, 외부 문서는 우리가 고칠 수 없으므로 규칙을 따로 정해야 한다. | [`sessions/2026-08-15-canon-quote-guard-plan.md`](sessions/2026-08-15-canon-quote-guard-plan.md) §9 · [`../qa/art-canon-move-review-issues.md`](../qa/art-canon-move-review-issues.md) N6 |
| F88 | 열림 | 낮음 | 📐 참조 규칙의 소급 적용 — 앵커 없는 참조 103곳·`정본:` 줄 없는 세션 62개 | 규칙은 신규부터 적용하기로 했고(`spec/docs-references.md` §13) 기계는 §7만 신규를 강제한다. 남은 소급분 셋의 규모와 세는 방법은 그 문서 §13 표가 든다(수치는 슬라이스마다 늘므로 집을 때 다시 잰다). 한 슬라이스에 안 담기므로 대상 문서를 건드릴 때 함께 고치는 것이 기본이고, 일괄 작업으로 집으려면 앵커 쪽부터 — 앵커가 붙으면 제목이 바뀔 때 `wf check-links`가 빨간불을 낸다. | `sessions/2026-08-18-docs-references-plan.md` §8 |
...
| F93 | 열림 | 중 | 🔧📐 6단계 절차 1번(QA 체크박스 + 통과 근거)을 기계가 안 본다 | [`workflow/verification.md`](workflow/verification.md)가 GREEN 직후 첫 항목으로 「자동 검증 항목을 `[x]`로 바꾸고 섹션 머리에 통과 근거(피처 N/N · 전체 M/M · 통과 커밋 SHA)를 적는다」를 걸고 스스로 **「자주 빠뜨리는 단계라 앞에 둔다」**고 적어 두었는데, 그 자각이 절차 문장에만 있고 게이트에는 없다. `check-qa`는 미확정 태그만 보므로 체크박스가 전부 비어 있어도 `pass review`가 통과한다 — `feat/eol-policy`가 둘 다 빠뜨린 채 `user-verification`까지 갔고 사용자가 문서를 열어 보고서야 잡혔다. 실측으로도 `*-test.md` 55개 중 통과 근거 줄이 있는 것은 32개뿐이고, 직전 `feat/docs-references`는 체크박스만 하고 근거 줄은 빠뜨려 **절차 1번을 절반만 지킨** 상태다. 막는 자리는 `pass review` 앞이고, 만들 때 갈래가 둘이다 — ① **절 범위 판별이 필요하다.** §5 수동 체크리스트의 `[ ]`는 사용자가 7단계에 채우는 것이라 정상이므로, 자동 검증 절만 골라 재야 한다(현재 두 절의 제목 문구가 문서마다 흔들린다). ② **요구하는 SHA가 머지 뒤 조회되지 않는다.** 브랜치 커밋 SHA는 squash 머지로 사라지므로(같은 뿌리를 `F76`이 이전 문서 찾기에서, 이 슬라이스 리뷰 `I1`이 정본 주석에서 각각 만났다) 무엇을 적게 할지를 함께 정한다 — PR 번호가 후보다. | [`../qa/eol-policy-test.md`](../qa/eol-policy-test.md) §4 |
