...
- **최초 작성:** 2026-09-16
- **상태:** CONFIRMED
- **이력:** 2026-09-17 — §5 외곽선의 미확인 둘을 노드 그룹을 읽어 확정(밀어내는 거리 = 폭 값, 화면 좌표 모드 미연결) · VRoid 기본 외곽선 값 · Line Art 굵기 단위 추가 · 2026-09-16 — 신설
...
아래는 2026-09-17에 노드 그룹 `VRM Add-on MToon 1.0 Outline Geometry Revision 1`을 읽어 확인한 것이다(애드온 v4.7.1 · Blender 5.2.1).

- **`worldCoordinates`에서 밀어내는 거리는 폭 값과 같다.** `Outline Width Factor`가 `Extrude Mesh`의 `Offset Scale`에 그대로 들어간다(폭 곱하기 텍스처가 있으면 그 초록 채널을 곱한다). 방향은 법선을 오브젝트 배율의 절댓값으로 나눈 것이라, 오브젝트가 배율돼 있어도 세계 거리가 폭 값(m)이다. 493px 캔버스(441px/m)에서 2.75px를 겨냥하면 폭 0.0062다.
- **`screenCoordinates`는 노드에 연결돼 있지 않다.** 노드 그룹에 `Outline Width Mode` 소켓이 있지만 어느 노드에도 이어지지 않는다. 그래서 어느 모드든 위와 같이 세계 거리로 밀어낸다. 직교 카메라에서는 세계 거리와 화면 픽셀이 비례하므로 이 굽기에는 상관없지만, 명세대로 화면 비율을 기대하고 값을 주면 엉뚱한 굵기가 나온다.
- **VRoid가 내보내는 외곽선 값은 폭 0.0008(0.8mm = 0.35px), 색 (0.061, 0.009, 0.014), 조명 혼합 0이다.** 기본 렌더에 외곽선이 안 보이는 이유가 이것이다.

### 5.1 Line Art (Blender 5.2)

Grease Pencil v3의 Line Art 모디파이어(`GREASE_PENCIL_LINEART`)는 굵기를 `thickness`가 아니라 **`radius`(m, 기본 0.0025)** 로 받는다. 직교 카메라에서 픽셀 굵기는 `2 × radius × (px/m)`라, 493px 캔버스에서 2.75px는 반지름 0.0031이다. `bpy.ops.object.grease_pencil_add(type='LINEART_SCENE')`가 백그라운드에서도 서고, 만든 오브젝트에는 `Black` 재질과 Line Art 모디파이어가 붙어 있다. 선 종류는 `use_contour` · `use_crease`(`crease_threshold`, 라디안) · `use_material` · `use_intersection` · `use_loose`이고, 가림 단계는 `level_start` · `level_end`다.
