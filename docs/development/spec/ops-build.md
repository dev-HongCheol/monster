...
- **이력:** 2026-08-13 — `spec/`으로 이전(`build-and-distribution.md` → `ops-build.md`). §4의 포커스 유실 진단이 세션 문서를 링크하던 것을 끊고 결론을 이 문서가 직접 들게 했다
...
**아직 결정된 것은 아니다.** 로드맵이 플랫폼에 대해 적어 둔 것은 이 한 줄이 전부다.

> **플랫폼:** v1 = itch.io 웹, v2 = Steam PC. 모바일 비목표.
> — [`roadmap.md`](../../planning/roadmap.md) 「플랫폼」 줄

배포 형태를 네이티브로 한다는 기록은 어디에도 없다. 지금 확인된 것은 **선택지가 실재한다**는 사실까지다.
