...
**먼저 서버를 내린다.** 브레인이 PGLite라 단일 쓰기인데 Claude Code가 띄운 `gbrain serve`가 세션 내내 DB를 잡고 있어서, 별도 CLI 프로세스는 같은 DB를 열지 못한다. **읽기 전용 명령까지 막히고, 그런데도 종료 코드가 0이다** — 잠금 안내를 뱉고 정상 종료하므로 실패한 줄 모르고 지나간다. 실제로 이 절차가 그 이유로 19일 동안 조용히 건너뛰어졌다(2026-07-25 → 2026-08-13).
...
그 PID를 종료한 뒤 동기화 두 줄을 돌린다. **윈도우에서는 두 줄 모두 `--full`이 필요하다** — 증분 동기화가 경로 검사 버그로 모든 파일을 거부하기 때문이다(§5.6). `--full`이라도 내용이 안 바뀐 파일은 해시로 걸러지므로 실제 비용은 증분과 비슷하다(179개 문서 중 6개가 바뀐 경우 11초).
