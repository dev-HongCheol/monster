...
### 이번에 건드리지 않은 주석은 손대지 않는다

막으려는 사고가 구체적이다. **한 함수를 고치거나 새 함수를 넣으면서, 같은 파일의 다른 함수에 붙어 있던 주석이 딸려 지워지는 일**이 실제로 있었다. 파일을 통째로 다시 쓸 때 나기 쉽고, diff를 대충 보면 눈에 안 띈다. 그렇게 사라진 주석은 대개 코드만 봐서는 복원되지 않는 이유를 들고 있어서, 다음 사람이 같은 판단을 처음부터 다시 해야 한다.

그래서 기준은 주석이 맞느냐 틀리냐가 아니라 **이번 변경이 그 코드를 건드렸느냐**다.

| 이번 변경과의 관계 | 처리 |
|---|---|
| 건드리지 않은 코드의 주석 | **손대지 않는다.** 이 줄이 규칙의 본체다 |
| 바꾼 코드의 주석 | **함께 갱신한다.** 동작·파라미터·불변식이 바뀌면 주석도 바뀐다(`@param` 포함) |
| 지운 코드의 주석 | **함께 지운다.** 없어진 동작을 설명하는 주석은 보존이 아니라 거짓말이다 |
| 건드리지 않은 코드인데 주석이 틀렸다 | 슬라이스 밖이므로 고치지 말고 **언급만 한다**(「행동 규칙」) |

둘째 줄을 빠뜨리면 첫째 줄이 "주석은 영원히 그대로"로 잘못 읽힌다. **코드가 바뀌었는데 주석이 남아 있는 것은 보존이 아니라 방치다.** 틀린 주석은 없는 주석보다 나쁘다 — 없으면 코드를 읽지만, 틀려 있으면 코드를 안 읽는다.

같은 설명이 코드·테스트·문서에 복사돼 있으면 **함께 고친다.** 한 곳만 고치면 나머지가 낡은 채로 남아 다음 사람이 어느 쪽을 믿어야 할지 모르게 된다.
...
**틀렸거나 낡은 주석은 없느니만 못하다** — 사람도 도구(AI)도 주석을 코드보다 먼저 믿기 때문이다. 그래서 같은 설명이 코드·테스트·문서에 흩어져 있으면 함께 고친다.
