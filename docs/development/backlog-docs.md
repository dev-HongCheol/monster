...
| F108 | 열림 | 중(플레이북 개정 슬라이스) | 📐 귀신 생성 요구사항을 플레이북 정본으로 승격 | `feat/blender-3d-gate` 2라운드 G2가 귀신 표본 두 장을 만들려고 화풍 조건을 GPT가 읽을 형태로 옮겨 적었는데, 그 문서가 세션에 남아 있다. **적 로스터 12종을 생성할 때 같은 조건을 다시 쓰므로** 거기 두면 다음 사람이 시점 기록을 명세로 읽는다. 받을 자리는 `art-generation-playbook.md`인데, 그 문서가 fal.ai Sandbox 화면과 플레이어 2D 생성 전제로 쓰여 있어(`fal` 참조 29곳) 지금은 GPT 웹 경로를 받지 못한다. **그래서 플레이북의 fal → GPT 웹 전환과 같은 슬라이스에서 집는다.** 그때 플레이어 절을 적·마법·맵으로 좁히는 F105 ①과도 겹친다. | [`sessions/2026-09-15-blender-3d-gate-round2-g2-toon-ghost-brief.md`](sessions/2026-09-15-blender-3d-gate-round2-g2-toon-ghost-brief.md) §5 |
