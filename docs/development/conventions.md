...
수정·기능 추가를 하면서 **이미 있는 주석을 노이즈로 보고 걷어내지 않는다.** 그 주석은 대개 코드만 봐서는 복원되지 않는 이유를 들고 있고, 지우면 다음 사람이 같은 판단을 처음부터 다시 해야 한다.

다만 **유지한다가 그대로 둔다는 뜻은 아니다.** 세 갈래로 갈린다.

| 상태 | 처리 |
|------|------|
| 여전히 맞다 | 그대로 둔다 |
| 코드와 어긋난다 | **고친다.** 파라미터가 바뀌었으면 `@param`도 함께 갱신한다 |
| 설명하던 코드가 사라졌다 | **주석도 함께 지운다** |

마지막 줄이 「삭제 금지」의 예외처럼 보이지만 아니다. 없어진 동작을 설명하는 주석은 보존이 아니라 **거짓말**이고, 다음 사람은 그것을 현재 명세로 읽는다. 틀린 주석은 없는 주석보다 나쁘다 — 없으면 코드를 읽지만, 틀려 있으면 코드를 안 읽는다.
