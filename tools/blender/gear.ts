...
const VIEWS = [
  ...
  // 왼쪽 측면 — 270°면 캐릭터의 왼쪽(+X)이 카메라(-Y)를 정확히 보는 순수 측면이고, 300°는 거기서 정면
  // 쪽으로 30° 튼 각이다. 왼팔은 앞에서 방패에, 뒤에서 머리카락에 가려 이 방향에서만 보인다. 순수 측면으로
  // 구웠더니 사용자가 「정면으로 약간 튼 측면」을 원했다(2026-09-17). 기본 세 방향에는 안 들고 경우가 고른다
  { id: 'left', yaw: 300 },
] as const;
 ...
 *
 * **위팔 판은 뺐다.** 소매에 7mm로 붙이니 소매 끝을 금색으로 칠한 것처럼 보였고, 2cm 띄우고 범위를
 * 줄여도 「흰 티 양쪽 팔 끝에 금색 뭔가」로 읽혀 사용자가 전부 제거하라고 했다(2026-09-17). 사진의 두 번째
 * 판(위팔 판)은 그래서 없다. 왼팔은 앞에서 방패에, 뒤에서 머리카락에 가려서 이 경우만 왼쪽 측면(`views`)을
 * 함께 굽는다.
 *
 * **뿔은 존재만 알 수 있으면 된다(사용자 2026-09-17).** 긴 머리카락이 어깨 장비를 가려 앞 · 뒤에서는 거의
 * 안 보이는데, 느낌만 보면 되고 있다는 정도가 보이면 된다고 했다. 뿔을 키우거나 머리 모양을 바꾸지 않는다.
 *
...
export const CASES: readonly IGearCase[] = [
  ...
  },
  {
    ...
    checks: [
      ...
      '흰 상의 소매 끝에 금색이 남아 있지 않나 (왼쪽 · 3/4)',
      '보석이 그늘에 묻히지 않나',
      '720p에서 보석 · 뿔 · 끈이 남나',
    ],
    spec: armor(),
