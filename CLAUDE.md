...
- **정본은 결정 기록(세션 문서·ADR)을 링크하지 않는다.** 정본은 현재 결론과 그 이유를 스스로 들고, 이력이 필요하면 정본 안의 이력 절에 **날짜와 무엇이 바뀌었는지**만 남긴다. 세션 파일명이 날짜로 시작하므로 링크가 없어도 `sessions/2026-08-04-*`로 찾아갈 수 있고, 링크와 달리 날짜는 낡거나 깨지지 않는다. 링크는 **결정 기록 → 정본** 한 방향으로만 건다(세션 문서 머리말의 `정본:` 줄). 정본이 반대 방향으로 이으면 결정이 바뀔 때마다 그 세션 문서를 찾아가 낡음을 표시해야 하는데 그 추적은 매번 놓친다 — `art-direction.md` 머리말이 스타일 확정 기록을 가리키고 있어서, 사용자가 링크를 타고 들어가 그 문서의 폐기된 외형 명세("뾰족 모자·긴 로브")를 현재 명세로 읽은 일이 실제로 있었다(2026-08-08). 한 결론이 세션 문서 여럿에 걸쳐 있어도 정본에 출처를 나열하지 않는다. **현재 결론 한 문장으로 다시 쓴다** — "A였다가 B가 됐다"를 정본에 적으면 그게 이력 서술이 되어 다시 세션 문서를 붙들게 된다.
