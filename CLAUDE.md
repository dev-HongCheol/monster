...
16. 사용자: **`PR 승인`** 입력 → AI는 (1) **7단계 테스트 중 Cocos가 생성한 모든 신규 `.meta`(로직·데이터·씬 자산 포함)를 먼저 커밋·push**하고 (2) `pnpm wf approve-pr` 실행 → `phase: "pr-ready"`
   ...
   - **문서 재검토는 승인 직후 9단계 17번에서 한다.** 백로그·정본 문서를 이번 슬라이스 기준으로 훑어 줄이고 고치는 단계이며, 기준은 9단계의 **「문서 정리 규칙」**이다. `approve-pr`을 막지 않으므로 승인을 먼저 하고 Draft 해제 전에 처리한다.

#### 9단계: PR 머지 (AI 주도)
> Draft PR은 7단계 진입 시 이미 생성돼 있다. 이 단계는 **Draft 해제 + 머지**다.
17. **Draft 해제 전 — 문서 정리(필수).** 관련 세션 문서·플랜 파일을 완료 상태로 업데이트하고, **이번 슬라이스가 건드린 백로그·정본 문서를 아래 「문서 정리 규칙」대로 재검토**한다. 변경이 있었으면 push해 Draft PR 본문·diff를 최신화한다.
...
##### 문서 정리 규칙 (17번 — 머지 전 마지막 재검토)

자주 읽는 문서(두 백로그·`docs/design/` 정본·이 파일)는 슬라이스마다 조금씩 자란다. 한 슬라이스 안에서는 티가 안 나지만 쌓이면 다음 슬라이스의 조회 비용이 된다 — 2026-08-06에 재 봤을 때 두 백로그 조회만 57.6k 토큰이었고, 그 부피의 38%가 항목 **여섯 개**에 몰려 있었다. 그래서 머지 직전에 **이번 슬라이스가 건드린 문서만** 아래로 훑는다(전수 점검이 아니다).
