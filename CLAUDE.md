...
- **플레이어 아트는 v2 최종본 기준으로 판정한다(2026-08-24 사용자 결정).** 지금 뽑는 이미지는 v1용 임시본이 아니라 **스킨까지 얹을 v2 최종본**이다. 그래서 재생성의 이유가 되는 것은 「v2에서 파츠로 잘라 층을 쌓을 수 있는가」뿐이고, **「지금 화면에서 어색한가」는 이유가 못 된다.** 생성이 유료이고 fal.ai가 재현을 보장하지 않아, 근거 없는 회차는 비용과 인물 동일성을 함께 건다. 회차를 제안하기 전에 항목마다 **v2에서 무엇이 깨지는지**를 적어 보고, 안 나오면 다음 캐릭터 턴어라운드에서 공짜로 받을 항목으로 돌린다.
- **규칙을 근거로 사용자의 지적을 방어하지 않는다.** 산출물이 이상하다는 말을 들으면 그것을 만든 규칙이 옳다는 것으로 답하지 말고, 그 **규칙이 선 전제**부터 확인한다. 2026-08-24에 「진행 중 문서는 PR을 올릴 때 굳는다」를 근거로 「정상이다」라고 답했는데, 실제로는 Draft PR이 검증 전에 올라가는 이 프로젝트에서 그 선 자체가 틀려 있었다(`docs/development/spec/docs-references.md` §9에서 머지로 옮겼다).
- **아트 지적은 사람의 눈이 정본이다.** "팔이 안 맞다"는 말을 **실측으로 재확인하지 않는다** — 잴 값은 「맞느냐」가 아니라 「어디가 원인이냐」다. 재야 하면 한 줄로 묻고 진행하라면 건너뛴다. 확인만 반복하면 다음 수가 밀린다.
