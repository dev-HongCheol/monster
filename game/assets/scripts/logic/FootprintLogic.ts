 ...
 * 올려 원점 좌표로 되돌린 뒤 ③ 아레나를 클램프한다. 어긋났을 때 눈에 보이는 실패는 둘이다:
 * ②를 빼먹으면 캐릭터가 매 프레임 오프셋만큼 가라앉고, ③까지 발밑 좌표에서 하면 **아레나
 * 경계가 통째로 오프셋만큼 밀린다**. 뒤엣것은 경계에 그려진 벽이 없어 화면에 아무 표시도 나지
 * 않으므로, 자동 테스트가 유일한 그물이다(리뷰의 뮤테이션 실험으로 확인 — 2026-07-25).
 * ①을 출발·목적지 중 한쪽에만 거는 실수는 **증상이 없다**: 한 프레임 이동 거리(약 3px)가 반지름
 * 25보다 훨씬 짧아 `resolveCircleMove`의 서브스텝이 1이라 결과가 목적지로 수렴하고, 차이는 dt가
 * 크게 튀어 서브스텝이 여러 개로 갈릴 때의 경로에만 남는다. 그래서 이 실수는 테스트로도 못 잡고,
 * 잡을 가치도 없다 — 여기 적어 두는 이유는 "튀는 증상이 날 것"이라 오해하고 없는 버그를 쫓지
 * 않게 하기 위해서다.
 *
 ...
 * 클램프라 "플레이어는 아레나 밖으로 절대 안 나간다"는 불변식이 순서에 상관없이 성립한다.
 * 순서를 뒤집으면 벽 근처에서 장애물에 밀린 위치가 클램프를 통과하지 않아 벽을 뚫을 수 있다.
 *
