 ...
 * 올려 원점 좌표로 되돌린 뒤 ③ 아레나를 클램프한다. 어긋났을 때의 증상은 셋 다 다르고, 그중
 * 하나만 조용하다(아래는 리뷰의 뮤테이션 실험으로 실측한 것 — 2026-07-25).
 ...
 * - ①을 **목적지에만** 걸면 접촉·정지 위치는 옳게 나온다(마지막 서브스텝의 끝점이 옳은 좌표라서).
 *   벽을 따라 미끄러지는 **경로**만 어긋나는데 실측 최대 편차가 약 12px이고 정지 위치가 같아
 *   눈에 띄지 않는다. 그래서 이 하나만 테스트하지 않는다 — 안 잡는 것이 아니라 고정할 관측
 *   가능한 결과가 없어서다.
 *
 ...
 * @param arena 아레나 크기 — null이거나 width가 0 이하·비유한이면 클램프하지 않는다(맵 로드 전)
