...
function segmentCircleEntry(
  ...
  // 첫 진입은 [0,1] 안에 있어야 유효하다. 선분이 원에서 **멀어지는** 방향(b>0)이면 두 근이 모두
  // 음수라, 원이 선분 뒤에 있는데도 t1을 그대로 돌려주면 소비처가 "거의 즉시 진입"으로 오해한다.
  // 그래서 t1<0도 -1로 접는다(segmentRectEntry가 같은 경우 -1을 주는 것과 맞춘다 — 헤더 주석의
  // "같은 역할"이 성립하려면 필요하다). t1>1이면 원이 선분 끝 너머라 이 이동으로는 못 닿는다.
