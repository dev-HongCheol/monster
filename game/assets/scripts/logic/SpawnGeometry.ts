...
//
// 뷰 사각형은 플레이어가 아니라 **카메라**를 중심으로 잡는다 — 화면에 무엇이 보이는지를 정하는 것이
// 카메라이기 때문이다. 카메라는 아레나 벽에서 멈추므로(ArenaLogic.cameraFollowPosition), 플레이어가
// 벽에 붙으면 플레이어는 화면 중앙이 아니라 벽 쪽 가장자리에 선다. 그러면 벽 반대 방향으로는 화면이
// 플레이어보다 훨씬 멀리까지(최대 viewHalf만큼 더) 펼쳐지고, 플레이어에게서 "충분히 멀다"고 뽑은 점도
// 그 방향에서는 아직 화면 안이다 — 적이 플레이어 눈앞에서 튀어나온다.
//
