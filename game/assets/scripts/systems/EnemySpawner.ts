...
export class EnemySpawner extends Component {
   ...
   *
   * 기준은 플레이어가 아니라 **카메라**다. 카메라는 아레나 벽에서 멈추므로, 플레이어가 벽에 붙으면
   * 플레이어는 화면 중앙이 아니라 벽 쪽 가장자리에 선다. 그러면 벽 반대 방향으로는 화면이 플레이어
   * 보다 멀리까지 펼쳐져, 플레이어 기준으로 뽑은 점이 그 방향에서는 아직 화면 안이다 — 적이 눈앞에서
   * 튀어나온다.
   *
