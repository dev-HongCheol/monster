 ...
 * 아레나를 클램프한다.
 *
 * 여기 테스트가 잡는 실패는 둘이다. 되돌리기를 빼먹으면 캐릭터가 매 프레임 오프셋만큼
 * 가라앉고(눈에 띄는 실패), **클램프까지 발밑 좌표에서 하면 아레나 경계가 통째로 오프셋만큼
 * 밀린다.** 뒤엣것은 경계에 그려진 벽이 없어 화면에 아무 표시도 나지 않는다 — 사람이 눈으로
 * 잡을 수 없는 실패라, 여기 테스트가 유일한 그물이다. (오프셋을 출발·목적지 중 한쪽에만 거는
 * 실수는 증상 자체가 없어 테스트하지 않는다 — 이유는 `FootprintLogic`의 JSDoc에 적었다.)
 */
