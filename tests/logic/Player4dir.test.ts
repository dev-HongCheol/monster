 ...
 * 아레나를 클램프한다.
 *
 * 여기 테스트가 잡는 실패는 둘이다. 되돌리기가 상쇄되지 않으면 캐릭터가 매 프레임 오프셋만큼
 * 가라앉거나 떠오르고(빈 벌판 테스트가 즉시 잡는다), **클램프까지 발밑 좌표에서 하면 아레나
 * 경계가 통째로 오프셋만큼 밀린다.** 뒤엣것은 경계에 그려진 벽이 없어 화면에 아무 표시도 나지
 * 않는다 — 사람이 눈으로 잡을 수 없는 실패라, 여기 테스트가 유일한 그물이다. (오프셋을
 * **목적지에만** 거는 실수 하나는 멈추는 위치가 옳게 나오고 경로만 어긋나서 여기서 다루지
 * 않는다 — 이유는 `FootprintLogic`의 JSDoc에 적었다.)
 */
describe('resolveMoveAtFootprint — 발밑에서 풀고 원점으로 되돌리는 이동 해소', () => {
  ...
  it('아레나 크기가 비유한이면 클램프하지 않는다', () => {
    // 아레나는 맵 JSON의 `size`에서 오므로 런타임 값이 숫자라는 보장이 타입에 없다. NaN이
    // 흘러들면 `NaN <= 0`이 false라 "크기가 있다"로 통과해 버리는데, 그 뒤 클램프는 x는
    // 그대로 두고 y만 반쪽짜리 경계로 잘라 플레이어를 화면 밖 어딘가로 옮긴다.
    const out = resolveMoveAtFootprint(
      { x: 700, y: 900 },
      { x: 720, y: 900 },
      RADIUS,
