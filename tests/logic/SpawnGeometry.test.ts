 ...
 * **카메라**를 중심으로 잡는다 — 카메라는 아레나 벽에서 멈추므로, 플레이어가 벽에 붙으면 플레이어는
 * 화면 중앙이 아니라 벽 쪽 가장자리에 선다. 그러면 벽 반대 방향으로는 화면이 플레이어보다 멀리까지
 * 펼쳐지고, 플레이어에게서 "충분히 멀다"고 뽑은 점도 그 방향에서는 아직 화면 안이다 — 적이 플레이어
 * 눈앞에서 튀어나온다.
 *
...
describe('offViewSpawnPoint — 사후조건 (속성 테스트)', () => {
  ...
  it('카메라 기준 회귀 가드 — 스폰 사각형의 중심은 플레이어가 아니라 카메라다', () => {
    // 플레이어가 벽에 붙으면 카메라는 클램프돼 플레이어와 다른 곳에 있다. 이때 플레이어 기준으로
    // 뽑으면 벽 반대 방향의 점이 화면 안에 들어와(그쪽으로 화면이 더 넓다) 회귀가 재발한다.
