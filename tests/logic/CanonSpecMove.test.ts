 ...
 * 정본 이전 슬라이스의 이동 단언.
 *
 * **여기 있는 것은 회귀망이 아니라 일회용 게이트다.** 이동이 끝나면 영구히 초록이고 다시는
 * 깨지지 않는다 — `wf ready-impl`이 요구하는 RED를 만들고, 이동이 계획대로 끝났는지 한 번
 * 재는 것이 역할이다. 앞으로 계속 회귀를 잡을 그물(링크·앵커·정본 인덱스)은 이름이 슬라이스에
 * 묶이지 않은 `DocLinks.test.ts`와 `CanonDoc.test.ts`에 둔다.
 */
