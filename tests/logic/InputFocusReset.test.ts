 ...
 * 정규화된 대각선 성분.
 *
 * 구현과 이 테스트가 같은 `Math.SQRT1_2`를 쓰므로 아래 부호 단언은 정확 비교로 통과한다.
 * 다만 이 값은 이전에 쓰던 `Vec3.normalize()`의 결과와 **같지 않다** — 그쪽은
 * `1 / Math.sqrt(2)`를 만들어 곱하고, 그 값이 여기서 1 ULP(약 1.1e-16) 어긋난다. 그래서
 * "정규화 결과"라는 성질 자체를 재는 단언(길이가 1인지 등)은 `toBeCloseTo`로 비교한다.
 */
