/**
 * 마크다운 링크·앵커를 형태로 검사하는 순수 로직 — 디스크를 읽지 않는다.
 *
 * 보는 것은 하나다. 문서가 거는 링크가 실재하는 파일과 절을 가리키는가(`findBrokenLinks`).
 *
 * **인용 형태 검사가 여기 함께 있었다(2026-08-15~08-19).** 정본이 다른 정본의 문장을 인라인으로
 * 박아 넣은 자리를 찾는 `findInlineCanonQuotes` 일습이 뒤쪽 212줄을 차지했는데, 형태만 보는 검사라
 * 막으려던 사고(복사된 문장이 낡아 거짓이 되는 것)를 실제로는 못 잡는다는 판단으로 걷어냈다.
 * 규칙 자체는 `docs/development/spec/docs-writing-style.md`에 남아 코드 리뷰가 받는다.
 * 경위는 `docs/development/sessions/2026-08-19-docs-guard-cut-plan.md` §3이 든다.
 *
